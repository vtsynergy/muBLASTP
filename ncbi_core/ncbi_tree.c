                   ...
                   Int2* ret_status)
{
    ...
    *ret_status = 0;
    ...
    //fprintf(stderr, "add node: index: %d\n", new_index);
                       ...
                       Int4 region_start, Int4 region_end, Int2* retval)
{
    ...
    new_index = s_IntervalNodeInit(tree, 1, eIntervalTreeNeither, retval);
    if(*retval != 0)
       return 0;
...
Blast_IntervalTreeFree(BlastIntervalTree *tree)
{
    ...
    int ii;
    for(ii = 0; ii < tree->num_used; ii++)
    {
        free(tree->nodes[ii].hsp);
    }
                             ...
                             enum EIntervalDirection which_end)
{
    ...
    while (1) {
        ...
        while (tmp_index != 0) {
            ...
            //fprintf(stderr, "in_hsp: q: %d - %d s: %d - %d tmp_index: %d leftptr: %d\n", 
            //in_hsp->query.offset, in_hsp->query.end, in_hsp->subject.offset, in_hsp->subject.end, tmp_index, next_node->leftptr);
            ...
            {
                //fprintf(stderr, "s_MidpointTreeHasHSPEndpoint: best_hsp\n");
                return TRUE;
            }
    ...
    while (1) {
        ...
        if (tmp_index != 0) {
            if (s_MidpointTreeHasHSPEndpoint(tree, tmp_index, in_hsp, 
                ...
                //fprintf(stderr, "s_MidpointTreeHasHSPEndpoint: tmp_index: %d\n", tmp_index);
                return TRUE;
            }
        }
                        ...
                        EITreeIndexMethod index_method)
{
    ...
    //fprintf(stderr, "tree add: region_start: %d region_end: %d\n", region_start, region_end);
    ...
    //fprintf(stderr, "region: %d - %d num_used: %d\n", region_start, region_end, tree->num_used);
    //int ii;
    //for(ii = 0; ii < tree->num_used; ii++)
    //{
    //fprintf(stderr, "nodes %d leftend: %d rightend: %d leftptr: %d midptr: %d rightptr: %d hsp %p\n", ii, nodes[ii].leftend, nodes[ii].rightend, nodes[ii].leftptr, nodes[ii].midptr, nodes[ii].rightptr, hsp);
    //}
