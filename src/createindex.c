...
int4 main(int4 argc, char *argv[]) {
  ...
  if (argc != 2 && argc != 4) {
    fprintf(stderr,
            "Useage: createindex <Database> [Word size] [Interval size]\n");
    exit(-1);
  }
  filename = argv[1];
  ...
  // Move through sequence file reading sequence codes and converting back to
  // ASCII sequences
  while (address < startAddress + fileSize) {
    // Record start position of sequence
    sequencePositions[sequenceCount] = address - startAddress;

    // Read two vbytes, the description location in the FASTA file and the
  ...
  if (fwrite(&headerData, sizeof(unsigned char), headerSize, indexFile) <
      headerSize) {
  ...
  if (fwrite(sequencePositions, sizeof(uint4), numberOfSequences, indexFile) <
      numberOfSequences) {
  ...
  if (fwrite(descriptionLocations, sizeof(uint4), numberOfSequences,
             indexFile) < numberOfSequences) {
  ...
  if (fwrite(listPositions, sizeof(uint4), numWords + 1, indexFile) <
      numWords + 1) {
    fprintf(stderr,
            "Error writing word offset position padding to index file %s\n",
            indexFilename);
    fprintf(stderr, strerror(errno));
    fprintf(stderr, "\n");
    fflush(stderr);
  ...
  codewordRangeSize =
      (300000000 / (sizeof(struct wordList) +
                    (6.0 * (float)numberOfLetters / (float)numWords /
                     (float)index_intervalSize)));
  printf("codewordRangeSize=%d\n", codewordRangeSize);

  // Pass through the collection considering words in the range between
  // fromCodeword and toCodeword
  fromCodeword = 0;
  toCodeword = 0;
  while (toCodeword < numWords) {
    ...
    //        wordOffsetPositions = index_wordOffsetPositions(fromCodeword,
    // toCodeword);
    ...
    while (codeword < toCodeword) {
      ...
      if (numOffsets > 0) {
        /*                int a = 0, b = 0;
           ...
           numOffsets, indexFile, b); fflush(stderr);
        */
        //                if (fwrite("abc", sizeof(unsigned char), 3, indexFile)
        // < 3)
        if (fwrite(offsets, sizeof(unsigned char), numOffsets, indexFile) <
            numOffsets) {
  ...
  fseek(indexFile, numberOfSequences * sizeof(uint4) * 2 + headerSize,
        SEEK_SET);
  ...
  printf("Size of index = %dMb + %dMb + %dMb + %dMb = ",
         numberOfSequences * 4 / 1048576, numberOfSequences * 4 / 1048576,
         ((numWords + 1) * 4) / 1048576, indexSize / 1048576);

  indexSize +=
      (numWords + 1) * 4 + numberOfSequences * 4 + numberOfSequences * 4;
  printf("%dMb\n", indexSize / 1048576);
