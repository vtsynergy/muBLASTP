...
// Prototypes
struct dpResults oldGappedScoring_dpBeforeSeed(unsigned char *subject,
                                               struct PSSMatrix PSSMatrix,
                                               struct coordinate seed,
                                               int4 dropoff);
struct dpResults oldGappedScoring_dpAfterSeed(unsigned char *subject,
                                              struct PSSMatrix PSSMatrix,
...
int4 oldGappedScoring_score(struct ungappedExtension *ungappedExtension,
  ...
  beforeDpResults =
      oldGappedScoring_dpBeforeSeed(subject, PSSMatrix, seed, dropoff);
  ...
  afterDpResults = oldGappedScoring_dpAfterSeed(
...
struct dpResults oldGappedScoring_dpBeforeSeed(unsigned char *subject,
  ...
  if (seed.subjectOffset >= oldGappedScoring_rowSizes) {
    ...
    // Malloc new rows
    oldGappedScoring_bestRow =
        (int4 *)global_malloc(sizeof(int4) * oldGappedScoring_rowSizes);
    oldGappedScoring_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * oldGappedScoring_rowSizes);
  }
  ...
  *insertQrow = insertS = match - parameters_openGap;
  ...
  while (queryPosition > PSSMatrix.matrix && rowDropoff < columnDropoff) {
    ...
    *insertQrow -= parameters_extendGap;
    ...
    //    if (dloc == 20877970)
    //        print(oldGappedScoring_bestRow, subject, rowDropoff,
    // columnDropoff);
  }
...
struct dpResults oldGappedScoring_dpAfterSeed(unsigned char *subject,
  ...
  *insertQrow = insertS = match - parameters_openGap;
  ...
  //    if (dloc == 94501334)
  //    print2(oldGappedScoring_bestRow + 1, subject, rowDropoff,
  // columnDropoff);
