...
//
// Code for performing a gapped alignment that records traceback information and
// uses
// the BLAST dropoff technique. This version is faster but less memory efficient
// than
...
// Prototypes
struct dpResults fasterGappedExtension_dpBeforeSeed(unsigned char *subject,
                                                    struct PSSMatrix PSSMatrix,
                                                    struct coordinate seed,
                                                    int4 dropoff);
struct dpResults fasterGappedExtension_dpAfterSeed(unsigned char *subject,
                                                   struct PSSMatrix PSSMatrix,
                                                   int4 dropoff,
                                                   int4 subjectLength);
struct trace
fasterGappedExtension_traceBeforeSeed(struct dpResults beforeDpResults,
                                      struct coordinate seed);
struct trace
fasterGappedExtension_traceAfterSeed(struct dpResults beforeDpResults,
                                     int4 queryLength);
struct trace fasterGappedExtension_joinTraces(struct trace beforeTrace,
...
fasterGappedExtension_build(struct ungappedExtension *ungappedExtension,
  ...
  beforeDpResults =
      fasterGappedExtension_dpBeforeSeed(subject, PSSMatrix, seed, dropoff);
  ...
  afterDpResults = fasterGappedExtension_dpAfterSeed(
...
struct dpResults fasterGappedExtension_dpBeforeSeed(unsigned char *subject,
  ...
  if (seed.subjectOffset >= fasterGappedExtension_rowSizes) {
    ...
    // Malloc new rows
    fasterGappedExtension_matchRow =
        (int4 *)global_malloc(sizeof(int4) * fasterGappedExtension_rowSizes);
    fasterGappedExtension_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * fasterGappedExtension_rowSizes);
    fasterGappedExtension_insertSrow =
        (int4 *)global_malloc(sizeof(int4) * fasterGappedExtension_rowSizes);
  }
  ...
  if (seed.queryOffset > fasterGappedExtension_numRows) {
    ...
    fasterGappedExtension_traceback = (unsigned char **)global_realloc(
        fasterGappedExtension_traceback,
    ...
    while (fasterGappedExtension_numRows < seed.queryOffset) {
      fasterGappedExtension_traceback[fasterGappedExtension_numRows] =
          (unsigned char *)global_malloc(sizeof(unsigned char) *
                                         (fasterGappedExtension_numColumns));
  ...
  tracebackRow =
      fasterGappedExtension_traceback + (queryPosition - PSSMatrix.matrix);
...
struct dpResults fasterGappedExtension_dpAfterSeed(unsigned char *subject,
  ...
  if (queryLength > fasterGappedExtension_numRows) {
    // Increase number of row pointers
    fasterGappedExtension_traceback = (unsigned char **)global_realloc(
        fasterGappedExtension_traceback, sizeof(unsigned char *) * queryLength);
...
// Debugging routine
void fasterGappedExtension_printBeforeRow(int4 *row, unsigned char *subject,
...
// Debugging routine
void fasterGappedExtension_printAfterRow(int4 *row, unsigned char *subject,
                                         unsigned char *rowDropoff,
                                         unsigned char *columnDropoff) {
  unsigned char *subjectPosition = subject;
  int4 *oldRow, *best;
