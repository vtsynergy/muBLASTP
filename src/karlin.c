...
//
// This code has been copied from blastkar.c and modified to make it work
// standalone
// without using implementations of mathematical functions in ncbimath.c and
// other
...
double BlastKarlinLHtoK(double *sfp, int4 min, int4 max, double lambda,
                        double H);
...
void BlastKarlinBlkCalc(double *scoreProbabilities, int4 min, int4 max) {
  ...
  /* Calculate K */
  BlastKarlin_K = BlastKarlinLHtoK(scoreProbabilities, min, max,
...
int4 BlastComputeLengthAdjustment(float K, float logK, float alpha_d_lambda,
                                  float beta, int4 query_length,
                                  uint4 db_length, int4 db_num_seqs,
