...
// Prototypes
struct dpResults tableGappedScoring_dpBeforeSeed(unsigned char *subject,
                                                 struct PSSMatrix PSSMatrix,
                                                 struct coordinate seed,
                                                 int4 dropoff);
struct dpResults tableGappedScoring_dpAfterSeed(unsigned char *subject,
                                                struct PSSMatrix PSSMatrix,
                                                int4 dropoff,
                                                int4 subjectLength);
...
int4 tableGappedScoring_score(struct ungappedExtension *ungappedExtension,
                              ...
                              unsigned char *packedSubject, int4 dropoff) {
  struct coordinate seed;
  ...
  beforeDpResults =
      tableGappedScoring_dpBeforeSeed(packedSubject, PSSMatrix, seed, dropoff);
  ...
  afterDpResults = tableGappedScoring_dpAfterSeed(
      choppedSubject, choppedPSSMatrix, dropoff, choppedSubjectSize);
  ...
  afterDpResults.best.subjectOffset =
      (afterDpResults.best.subjectOffset + seed.subjectOffset) * 4;
  ...
  if (parameters_verboseDloc == blast_dloc)
    printf("table[%d,%d](seed=%d,%d)\n", beforeDpResults.bestScore,
           afterDpResults.bestScore, seed.queryOffset, seed.subjectOffset * 4);
#endif
...
int4 tableGappedScoring_compareBlocks(const void *block1, const void *block2) {
  ...
  while (columnCarry <= tableGappedScoring_maxChange) {
    if (b1->tablePoint4er[columnCarry] > b2->tablePoint4er[columnCarry]) {
      return -1;
    } else if (b1->tablePoint4er[columnCarry] <
               b2->tablePoint4er[columnCarry]) {
      return 1;
    }
...
// Build the table used to perform nucleotide alignment where the subject is
// compressed
void tableGappedScoring_buildTable() {
  ...
  tableGappedScoring_maxChange =
      parameters_matchScore + parameters_bytepackExtendGap;

  range = tableGappedScoring_maxChange - tableGappedScoring_minChange + 1;

  maxCarry = range * range * range * range;

  // Declare memory to hold pointers into the table for every possible column
  // carry
  // and matchVector value
  tableBlocks = (struct tableBlock *)global_malloc(sizeof(struct tableBlock) *
                                                   maxCarry * 16);
  ...
  while (carryIn < maxCarry) {
    ...
    while (carryPosition < 4) {
      carryIns[carryPosition] =
          (tempCarryIn % range) + tableGappedScoring_minChange;
      tempCarryIn /= range;
      carryPosition++;
    }

    //        printf("Carry in = %d,%d,%d,%d\n", carryIns[0], carryIns[1],
    // carryIns[2], carryIns[3]);

    // For each of the possibly match vectors between a query character and
    // subject 4-char
    matchVector = 0;
    while (matchVector < 16) {
      ...
      while (columnCarry <= tableGappedScoring_maxChange) {
        ...
        carryOuts[0] =
            maximum3(tempColumnCarry - parameters_bytepackExtendGap,
                     carryIns[0] - parameters_bytepackExtendGap, matchScore) -
            tempColumnCarry;

        //				printf("Max(%d,%d,%d}\n", tempColumnCarry
        //-
        // parameters_bytepackExtendGap,
        //                                        carryIns[0] -
        // parameters_bytepackExtendGap,
        //                                        matchScore);
        ...
        while (carryPosition < 4) {
          ...
          /*				printf("Max(%d,%d,%d} - %d\n", tempColumnCarry
             ...
             parameters_bytepackExtendGap,
                                                         tempCarryIn +
             carryIns[carryPosition] - parameters_bytepackExtendGap,
                                                         tempCarryIn +
             matchScore,
                                                         tempColumnCarry);
        ...
        while (carryPosition > 0) {
          ...
          //                printf("* %d + %d Co=%d\n", range,
          // carryOuts[carryPosition] - tableGappedScoring_minChange, carryOut);
        }
  ...
  tableGappedScoring_tableResults =
      (char *)global_malloc(sizeof(char) * maxCarry);
  ...
  while (carryOut < maxCarry) {
    ...
    while (carryPosition < 4) {
      carryOuts[carryPosition] =
          (tempCarryOut % range) + tableGappedScoring_minChange;
      tempCarryOut /= range;
      carryPosition++;
    }

    //        printf("Carry out = %d,%d,%d,%d\n", carryOuts[0], carryOuts[1],
    // carryOuts[2], carryOuts[3]);
  ...
  qsort(tableBlocks, maxCarry * 16, sizeof(struct tableBlock),
        tableGappedScoring_compareBlocks);

  // Build the final table
  tableGappedScoring_tablePoint4ers =
      (uint2 *)global_malloc(sizeof(int2) * maxCarry * 16);
  tableGappedScoring_table =
      (uint2 *)global_malloc(sizeof(int2) * range * 16 * maxCarry);
  ...
  while (blockCount < maxCarry * 16) {
    ...
    if (previousTablePosition &&
        tableGappedScoring_compareBlocks(tableBlock,
                                         tableBlocks + blockCount - 1) == 0) {
      // Reuse the block
      ...
      // Record a new block
      tableGappedScoring_tablePoint4ers[matchVector * maxCarry + carryIn] =
          tablePosition - tableGappedScoring_minChange -
          tableGappedScoring_table;
  ...
  tableGappedScoring_backwardMatchVectors =
      (uint2 *)global_malloc(sizeof(int2) * 256);
  tableGappedScoring_forwardMatchVectors =
      (uint2 *)global_malloc(sizeof(int2) * 256);

  // For each possible result of xoring two byte-packed sequences
  xorResults = 0;
  while (xorResults < 256) {
    // Match vectors when performing dynamic programming backwards (before the
    // seed)
    matchVector = 0;
    if (!(xorResults & 0x3))
      matchVector |= 1;
    ...
    tableGappedScoring_backwardMatchVectors[xorResults] =
        matchVector * maxCarry;

    // Match vectors when performing dynamic programming forwards (after the
    // seed)
    matchVector = 0;
    if (!(xorResults & 0x3))
      matchVector |= 8;
  ...
  /*
      ...
      while (carryIn < maxCarry)
      {
          ...
          while (matchVector < 16)
          {
              ...
              while (columnCarry <= tableGappedScoring_maxChange)
              {
                  if (tableGappedScoring_tablePoint4ers[(carryIn << 4) |
     matchVector][columnCarry] !=
                      tableGappedScoring_tablePoint4ers2[(carryIn << 4) |
     matchVector][columnCarry])
                  {
                          printf("Error!\n");
                                  }
...
struct dpResults tableGappedScoring_dpBeforeSeed(unsigned char *subject,
  ...
  if (seed.queryOffset >= tableGappedScoring_rowSizes) {
    ...
    // Malloc new rows
    tableGappedScoring_bestRow =
        (int4 *)global_malloc(sizeof(int4) * tableGappedScoring_rowSizes);
  }
...
#ifdef VERBOSE
  if (parameters_verboseDloc == blast_dloc)
    gappedExtension_printBeforeRow(tableGappedScoring_bestRow,
                                   PSSMatrix.xorCodes, rowDropoff,
                                   columnDropoff);
#endif
  ...
  while (subjectPosition > subject && rowDropoff < columnDropoff - 1) {
    ...
    *bestRow -= parameters_bytepackExtendGap * 4;
    ...
    while (queryPosition >= rowDropoff) {
      ...
      matchVector = tableGappedScoring_backwardMatchVectors
      ...
      //            printf("columnCarry=%d matchVector=%d carryIn=(%d) ",
      // (*bestRow - previousBest), matchVector, carry);
      //			tableGappedScoring_printCarry(carry);
      // fflush(stdout);

      //			printf("[%d]", *bestRow - previousBest);
      // fflush(stdout);

      // Lookup value of carry-out in table
      carry = (tableGappedScoring_table +
               tableGappedScoring_tablePoint4ers[matchVector + carry])
          [(*bestRow - previousBest)];
  ...
  *bestRow = best = 0;
...
#ifdef VERBOSE
  if (parameters_verboseDloc == blast_dloc)
    gappedExtension_printAfterRow(tableGappedScoring_bestRow,
                                  PSSMatrix.xorCodes, rowDropoff,
                                  columnDropoff);
#endif
  ...
  while (subjectPosition < subjectEnd && rowDropoff > columnDropoff + 1) {
    ...
    *bestRow -= parameters_bytepackExtendGap * 4;
    ...
    while (queryPosition < rowDropoff) {
      // Get the match vector for query/subect at this position
      matchVector = tableGappedScoring_forwardMatchVectors
          [*queryPosition ^ *subjectPosition];
      ...
      //            printf("last=%d + %d carryOut=(%d) ",
      // tableGappedScoring_tableResults[carry], *bestRow, carry);
      //			tableGappedScoring_printCarry(carry);
      // fflush(stdout);
