...
//
// Array used by BLAST to detect two hits on the same diagonal, or in one-hit
// mode
...
void hitMatrix_initialize(int4 queryLength, int4 maximumSubjectLength,
...
void hitMatrix_reinitialize(int4 queryLength, int4 maximumSubjectLength,
                            unsigned char *startAddress) {
  int4 numDiagonals;
  unsigned char **minOffset, **offset, **maxOffset;
