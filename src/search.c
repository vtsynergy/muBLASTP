        ...
        if (currentWord) {
          ...
          do {
            ...
            if (*lastHit < address) {
              ...
              ungappedExtension = ungappedExtension_oneHitExtend(
                  PSSMatrix.matrix + queryOffset, address, PSSMatrix, subject);
  ...
  while (sequenceCount < numSequences) {
    ...
    if (subjectLength >= parameters_wordSize) {
      ...
      //// by wordLookupDFA to get the query positions of int4erest
        ...
        if (proteinLookup_query[codeWord].numQueryPositions > 0) {
          //// Calculate subject offset
            subjectOffset = address - subject;
          ...
          for(ii = 0; ii < proteinLookup_query[codeWord].numQueryPositions; ii++) {
            ...
            // printf("%d %d %d %d\n", queryOffset, sequenceCount,
            // subjectOffset, distance);
            ...
            if (distance >= parameters_A) {
              ...
              *lastHit = address;
              ...
              ungappedExtension = ungappedExtension_extend(
                  PSSMatrix.matrix + queryOffset, address, *lastHit, PSSMatrix,
                  subject);
...
void search_nucleotide(struct PSSMatrix PSSMatrix,
  ...
  unsigned char *subject, *sequenceEnd, prevByte, nextByte, rightMatches,
      leftMatches;
        ...
        if (nucleotideLookup_bitLookup[codeword >> 5] &
          ...
          if (tableEntry) {
            ...
            do {
              ...
              rightMatches =
                  PSSMatrix_packedRightMatches[nextByte ^ *subjectPosition];
              prevByte =
                  PSSMatrix.bytePackedCodes[queryOffset - previousByteDistance];
              leftMatches = PSSMatrix_packedLeftMatches
                  [prevByte ^ *(address - parameters_wordTableBytes)];
              ...
              if (rightMatches + leftMatches >= parameters_wordExtraLetters) {
                ...
                // Calculate the diagonal this hit is on
                diagonal =
                    (subjectOffset * 4 - queryOffset + hitMatrix_queryLength) &
                ...
                if (*lastHit < address) {
                  ...
                  ungappedExtension = ungappedExtension_nucleotideExtend(
                      queryOffset, subjectOffset, PSSMatrix, subject,
                      subjectLength);
...
// Search a nucleotide database using 1-hit extension mode with large wordsize >
// 14
void search_nucleotide_longWord(struct PSSMatrix PSSMatrix,
  ...
  wordLengthMinusOne = parameters_wordSize - 1;
  previousByteDistance =
      parameters_wordTableLetters + parameters_wordExtraBytes * 4 + 4;
  singleQueryOffset[1] = 0;
          ...
          if (tableEntry) {
            ...
            do {
              ...
              if (!extraBytesNeeded) {
                ...
                rightMatches =
                    PSSMatrix_packedRightMatches[nextByte ^ *(subjectPosition)];
                prevByte = PSSMatrix.bytePackedCodes
...
//                            	printf("Hit! dloc=%d\n", descriptionStart);
                  ...
                  // Calculate the diagonal this hit is on
                  diagonal = (subjectOffset * 4 - queryOffset +
                              hitMatrix_queryLength) &
                             hitMatrix_diagonalMask;
                  ...
                  if (*lastHit < address) {
                    ...
                    *lastHit = ungappedExtension_subjectEndReached;
                    ...
                    if (ungappedExtension) {
                      ...
                      // Create new alignment if needed
                      if (alignments_currentAlignment == NULL) {
                        alignments_createNew(descriptionStart,
                                             descriptionLength, subject,
...
// Search a nucleotide database using 1-hit extension mode, using a large word
// lookup table
        ...
        if (nucleotideLookup_bitLookup[codeword >> 5] &
          ...
          if (tableEntry) {
            if (tableEntry > 0) {
              ...
              // Multiple query positions
              queryOffsets =
                  nucleotideLookup_additionalPositions_large - tableEntry;
              queryOffset = *queryOffsets;
            }
...
/*
// Search a nucleotide collection using an inverted index to speed-up BLAST
search
void search_nucleotideIndex(struct PSSMatrix PSSMatrix, struct sequenceData*
sequenceData,
                            uint4 numSequences, uint4 tickFrequency)
{
        uint4 sequenceCount = 0, codeword;
...
leftMatches;
    unsigned char previousCode, *unpackedSubject;
        int4 subjectLength, subjectPosition, wordLengthMinusOne,
lastSubjectPosition;
        struct ungappedExtension* ungappedExtension;
        int4 *queryPositions, tableEntry, queryPosition, singleQueryPosition[2],
lastQueryPosition;
    int4 previousByteDistance;
    int4 diagonal;
    ...
    // TODO: Use size of volume instead of size of database
        ...
        sequencePositions = (uint4*)global_malloc(sizeof(uint4) *
readdb_numberOfSequences);
        descriptionLocations = (uint4*)global_malloc(sizeof(uint4) *
readdb_numberOfSequences);
...
subjectLength);
            ...
            // Read a third vbyte; the total length of sequence data including
wildcard info
...
coordinate->subjectNumber,
//               descriptionStart, coordinate->queryOffset,
coordinate->subjectOffset);
        ...
        if (*lastHit < address && ungappedExtension_checkHit(queryPosition,
subjectPosition,
                PSSMatrix, subject, subjectLength, index_wordSize +
index_intervalSize - 1))
        {
            #ifdef VERBOSE
            if (parameters_verboseDloc == descriptionStart)
                        printf("CheckHit score=%d\n", queryPosition,
subjectPosition, ungappedExtension_bestScore);
            #endif
...
void search_proteinSsearch(struct PSSMatrix PSSMatrix,
  ...
  while (sequenceCount < numSequences) {
    ...
    if (dpResults.bestScore >= blast_gappedNominalCutoff &&
      ...
      // Perform SW alignment in the reverse direction, to find the start of the
      // optimal alignment
      reverseDpResults = smithWatermanScoring_scoreReverse(
          PSSMatrix, subjectLength, subject, dpResults.best);

      // Collect traceback information and store alignment
      gappedExtension =
          smithWatermanTraceback_build(PSSMatrix, subjectLength, subject,
      ...
      if (reverseDpResults.bestScore != dpResults.bestScore ||
        ...
        fprintf(stderr, "Error: Forward and reverse Smith-Waterman alignment "
                        "scores do not match\n");
        exit(-1);
      }
      ...
      alignments_createNew(descriptionStart, descriptionLength, subject,
                           subjectLength, encodedLength);
      alignments_addGappedExtension(alignments_currentAlignment,
                                    gappedExtension);
      alignments_addFinalAlignment(dpResults.bestScore,
                                   alignments_currentAlignment);
    }
...
void search_nucleotideSsearch(struct PSSMatrix PSSMatrix,
  ...
  while (sequenceCount < numSequences) {
    ...
    // Perform SW score only
    dpResults =
        smithWatermanScoring_score(PSSMatrix, subjectLength, unpackedSubject);
    ...
    if (dpResults.bestScore >= blast_gappedNominalCutoff &&
      ...
      // optimal alignment
      reverseDpResults = smithWatermanScoring_scoreReverse(
          PSSMatrix, subjectLength, unpackedSubject, dpResults.best);

      // Collect traceback information and store alignment
      gappedExtension = smithWatermanTraceback_build(
          PSSMatrix, subjectLength, unpackedSubject, reverseDpResults.best,
          dpResults.best);
      ...
      if (reverseDpResults.bestScore != dpResults.bestScore ||
        ...
        fprintf(stderr, "Reverse Score=%d End=%d,%d\n",
                reverseDpResults.bestScore, reverseDpResults.best.queryOffset,
                reverseDpResults.best.subjectOffset);
        fprintf(stderr, "Traceback Score=%d End=%d,%d\n",
                gappedExtension->nominalScore, gappedExtension->queryEnd,
                gappedExtension->subjectEnd);
        //                exit(-1);
      }

      gappedExtension_score(gappedExtension);

      alignments_createNew(descriptionStart, descriptionLength, unpackedSubject,
                           subjectLength, 0);
      alignments_addGappedExtension(alignments_currentAlignment,
                                    gappedExtension);
      alignments_addFinalAlignment(dpResults.bestScore,
...
/*
// Testing code
void search_compareScoring(struct alignment* alignment, struct PSSMatrix
PSSMatrix)
{
        struct ungappedExtension* ungappedExtension;
...
alignment->subject);
        ...
        {
        score1
                = tableGappedScoring_score(ungappedExtension, PSSMatrix,
...
statistics_gappedNominalDropoff);
        ...
        score2
                        = nuGappedScoring_score(ungappedExtension, PSSMatrix,
alignment->subjectLength,
                                            alignment->subject,
statistics_gappedNominalDropoff);

        score4
                = bytepackGappedScoring_score(ungappedExtension, PSSMatrix,
alignment->subjectLength,
                        alignment->subject, statistics_gappedNominalDropoff);


                printf("UE=%d TableDriven=%d GappedScoring=%d Bytepacked=%d
Dloc=%d Slength=%d SeedQ=%d SeedS=%d\n",
                        ungappedExtension->nominalScore, score1, score2, score4,
            alignment->descriptionLocation, alignment->subjectLength,
                        ungappedExtension->seed.queryOffset,
...
alignment*)memBlocks_getCurrent(alignments_alignments)))
        {
