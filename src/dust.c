...
void dust_dustSequence(char *originalSequence) {
  ...
  while (count < sequenceLength - 2) {
    // Encode triplet
    sequence[count] = (sequence[count] << 4) | (sequence[count + 1] << 2) |
                      sequence[count + 2];
  ...
  for (windowStart = 0; windowStart < sequenceLength;
       windowStart += windowhalf) {
    windowLength = (int)((sequenceLength > windowStart + windowSize)
                             ? windowSize
                             : sequenceLength - windowStart);
    windowLength -= 2;

    //        printf("process window (length=%d, position=%d)\n", windowLength,
    // windowStart); fflush(stdout);
    ...
    if (chunk.score > cutoffScore) {
      ...
      if (regions && regions->to + linker >= chunk.start + windowStart &&
        ...
        currentRegion =
            (struct maskRegion *)global_malloc(sizeof(struct maskRegion));
...
void dust_processWindow(int windowLength, int windowStart, struct chunk *chunk,
                        unsigned char *sequence) {
  int chunkStart;
  ...
  for (chunkStart = 0; chunkStart < windowLength; chunkStart++) {
    //    	printf("wo1 (%d,%d)\n", windowLength-i, i);
    dust_processChunk(windowLength - chunkStart, sequence + chunkStart,
                      chunkStart, chunk);
  }
...
void dust_processChunk(int windowLength, unsigned char *sequence,
                       int chunkStart, struct chunk *chunk) {
  unsigned int sum;
