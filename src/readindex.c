...
struct listinfo *get_list_at(struct index_scanner *iscn, struct listinfo *info,
                             long offset) {
  struct vec *vector;
  unsigned long num, last_doc = -1;
  int c, d;
  FILE *idx = iscn->idx_file;
  unsigned long veclen, numdocs, occurs, last_offset;

  if (offset >= 0)
    fseek(idx, offset, SEEK_SET);

  /* There is nothing left to read; free structures */
  if (!(vbyte_read(idx, &numdocs) && vbyte_read(idx, &occurs) &&
  ...
  if (info->size < numdocs) {
    info->doc_numbers =
        realloc(info->doc_numbers, numdocs * sizeof(unsigned long));
    info->phrase_frequency =
        realloc(info->phrase_frequency, numdocs * sizeof(unsigned long));
    info->phrase_offsets =
        realloc(info->phrase_offsets, numdocs * sizeof(unsigned long *));
    memset(info->phrase_offsets + info->size, 0,
           (numdocs - info->size) * sizeof(unsigned long *));
    info->size = numdocs;
  }
  ...
  for (c = 0; c < numdocs; c++) {
    ...
    info->phrase_offsets[c] =
        malloc(info->phrase_frequency[c] * sizeof(unsigned long));
    memset(info->phrase_offsets[c], 0,
           info->phrase_frequency[c] * sizeof(unsigned long));
