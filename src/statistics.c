...
int4 statistics_querySize_multi[BATCH_SIZE];
int4 statistics_effectiveQuerySize;
int4 statistics_effectiveQuerySize_multi[BATCH_SIZE];
uint8 statistics_databaseSize = 0;
uint8 statistics_effectiveDatabaseSize;
uint8 statistics_searchSpaceSize;
uint8 statistics_searchSpaceSize_multi[BATCH_SIZE];
...
// Dropoff values
int4 statistics_ungappedNominalDropoff_multi[BATCH_SIZE];
int4 statistics_gappedNominalDropoff_multi[BATCH_SIZE];
int4 statistics_gappedFinalNominalDropoff_multi[BATCH_SIZE];
...
statistics_lookupPrecomputedParams(char *matrix, int2 startGap, int2 extendGap);

// Initialize statistics by calculating some global parameters
void statistics_initialize(struct PSSMatrix PSSMatrix, uint8 databaseSize,
  ...
  statistics_ungappedNominalDropoff =
      ceil(parameters_ungappedNormalizedDropoff * statistics_log2 /
           statistics_ungappedLambda);

  // Same for initial and final gapped extension dropoffs
  statistics_gappedNominalDropoff =
      floor(parameters_gappedNormalizedDropoff * statistics_log2 /
            statistics_gappedParams.lambda);
  statistics_gappedFinalNominalDropoff =
      floor(parameters_gappedFinalNormalizedDropoff * statistics_log2 /
            statistics_gappedParams.lambda);
  ...
  if (encoding_alphabetType == encoding_protein) {
    ...
    statistics_lengthAdjust =
        floor(statistics_calcLengthAdjust(numberOfSequences));
  }

  // Using length adjustment, calculate effective query, database length and
  // search space
...
float statistics_calcLengthAdjustNew(int4 numberOfSequences) {
  ...
  BlastComputeLengthAdjustment(
      statistics_gappedParams.K, statistics_gappedLogK,
      statistics_gappedParams.alpha / statistics_gappedParams.lambda,
      statistics_gappedParams.beta, statistics_querySize,
      statistics_databaseSize, numberOfSequences, &lengthAdjust);
...
int4 statistics_ungappedNormalized2nominal(float normalizedScore) {
  ...
  nominalScore = (normalizedScore * statistics_log2 + statistics_ungappedLogK) /
                 statistics_ungappedLambda;
...
float statistics_ungappedNominal2normalized(int4 nominalScore) {
  ...
  normalizedScore = (((float)nominalScore * statistics_ungappedLambda) -
                     statistics_ungappedLogK) /
                    statistics_log2;
...
// Calculate the evalue for a given ungapped normalizedScore
double statistics_ungappedCalculateEvalue_multi(float normalizedScore,
                                                int queryNum) {
  return statistics_searchSpaceSize_multi[queryNum] / pow(2, normalizedScore);
}

// Given an evalue (such as a cutoff) calculate the minimum ungapped nominal
// score needed to attain it
int4 statistics_ungappedEvalue2nominal(double evalue) {
  double normalizedScore;

  normalizedScore =
      log(statistics_searchSpaceSize / evalue) / (double)statistics_log2;
...
float statistics_gappedNominal2normalized_ncbi(int4 nominalScore) {
  ...
  double lambda = 0.0083437500000000005; 
  double gappedLogK = -3.1941832122778293;
  double scoreDivisor = 32;
  nominalScore = BLAST_Nint(((double) nominalScore) / scoreDivisor);
  normalizedScore = (((double)nominalScore * lambda * scoreDivisor) -
          gappedLogK)/NCBIMATH_LN2;

  return normalizedScore;
}

// Given a normalized score calculate the lowest gapped nominal score needed to
// achieve it
...
// Calculate the evalue for a given gapped normalizedScore
double statistics_gappedCalculateEvalue_multi(float normalizedScore,
                                              int queryNum) {
  return statistics_searchSpaceSize_multi[queryNum] / pow(2, normalizedScore);
}

// Given an evalue (such as a cutoff) calculate the minimum gapped nominal score
// needed to attain it
int4 statistics_gappedEvalue2nominal_multi(double evalue, int queryNum) {
  double normalizedScore;

  normalizedScore = log(statistics_searchSpaceSize_multi[queryNum] / evalue) /
                    (double)statistics_log2;

  //    printf("[%f,%f,%f,%f, %f]\n", normalizedScore, statistics_log2 *
  // normalizedScore, statistics_log2 * normalizedScore +
  // statistics_gappedLogK,(statistics_log2 * normalizedScore +
  // statistics_gappedLogK)
  //	       / (double)statistics_gappedParams.lambda,
  // statistics_gappedParams.lambda);
...
// Calculate minimum nominal score required to trigger gapping for nucleotide
// searches
int4 statistics_ungappedNucleotideTrigger(struct PSSMatrix PSSMatrix) {
  ...
  if (averageSubjectLength > PSSMatrix.length) {
    ...
    normalizedScore =
        log(averageSubjectLength * averageSubjectLength / evalue) /
        (double)statistics_log2;
  }
...
/***************************************************************************************/
/* For each matrix, gapStart, gapExtend - precomputed gapped lambda, K, H,
 * alpha, beta */
/***************************************************************************************/

#define NUM_PRECOMPUTED_PARAMS 60
static const struct statisticalParameters statistics_precomputedParams
    [NUM_PRECOMPUTED_PARAMS] = {
...
struct statisticalParameters
statistics_lookupPrecomputedParams(char *matrix, int2 startGap,
                                   int2 extendGap) {
  int4 count = 0;
  ...
  while (count < NUM_PRECOMPUTED_PARAMS) {
    ...
    if (strcmp(parameters.matrix, matrix) == 0 &&
        parameters.startGap == startGap && parameters.extendGap == extendGap) {
...
void statistics_calculateUngappedKarlinParameters(struct PSSMatrix PSSMatrix) {
  ...
  scoreProbabilities =
      (double *)global_malloc(sizeof(double) * (highest - lowest + 1));
  ...
  // Determine the number of regular letters (ie. non-ambigious codes) in the
  // query
  queryPosition = 0;
  ...
  // For each position in the query PSSMatrix that does not represent an
  // ambigious code
  queryPosition = 0;
  while (queryPosition < statistics_querySize) {
    //    	if (PSSMatrix.queryCodes[queryPosition] <
    ...
    if (PSSMatrix.queryCodes[queryPosition] != encoding_numRegularLetters) {
      ...
      while (subjectCode < encoding_numRegularLetters) {
        // Calculate probability that, if a residue was randomly chosen from the
        // subject database
        // and a position was randomly chosen from query, they would be
        // subjectCode and queryPosition
        // respectively
        probability =
            alphabetFrequencies[subjectCode] / (float)numRegularLettersInQuery;
  ...
  // Normalized probabilities by dividing them by the sum of the robinson
  // frequency values
  score = lowest;
  while (score <= highest) {
  ...
  if (statistics_ungappedK <= 0.0001) {
    fprintf(stderr, "Error: unable to calculate statistical significance for "
...
// Initialize statistics by calculating some global parameters
void statistics_initialize_multi(struct PSSMatrix PSSMatrix, uint8 databaseSize,
                                 int4 numberOfSequences, int queryNum) {
  // Record the size (number of letters) of query and subject database
  if (PSSMatrix.strandLength == 0)
    statistics_querySize = statistics_querySize_multi[queryNum] =
        PSSMatrix.length;
  else
    statistics_querySize = statistics_querySize_multi[queryNum] =
        PSSMatrix.strandLength;
  ...
  // Using length adjustment, calculate effective query, database length and
  // search space
  statistics_effectiveQuerySize_multi[queryNum] =
      statistics_querySize - statistics_lengthAdjust;
  statistics_effectiveDatabaseSize =
      statistics_databaseSize - numberOfSequences * statistics_lengthAdjust;
  statistics_searchSpaceSize_multi[queryNum] =
      statistics_effectiveQuerySize_multi[queryNum] *
      statistics_effectiveDatabaseSize;
}
