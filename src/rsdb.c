...
void rsdb_printSequence(uint4 whitespace, unsigned char *sequence,
                        uint4 length);
...
void rsdb_addChild(struct parent *parent, struct sequence *child,
                   int4 relativeOffset);
// Merge two lists in the hashtable and return the intersection
struct wordLocation *
rsdb_mergeLists(struct wordLocation *wordLocations1, uint4 *numWordLocations1,
...
int4 main(int4 argc, char *argv[]) {
  ...
  if (argc != 2 && argc != 6) {
    fprintf(stderr, "Useage: rsdb <database> [decoWordLength] [numIterations] "
                    "[decoQantum] [maxListLength]\n");
    exit(-1);
  }
  filename = argv[1];
  ...
  //    rsdb_simpleClusterSequences(sequences, sequenceNumber,
  // readdb_numberOfLetters);
  ...
  //    rsdb_clusterSequences(sequences, sequenceNumber, readdb_numberOfLetters,
  // filename);
...
void rsdb_writeClusters(char *filename, struct sequence *sequences) {
  ...
  while (sequenceNumber < readdb_numberOfSequences) {
    if (sequences[sequenceNumber].parent == NULL ||
        ...
        sequences[sequenceNumber].parent->sequence ==
      ...
      if (sequenceSize >= wordLength) {
        ...
        while (offset < sequenceSize - wordLength) {
          ...
          if (hashCounter1 == NULL || submatches >= 2) {
            ...
            //	                    if (debug)
            //                        printf("New hash [%d] %u (submatches=%d)
            // winPos=%d\n", offset, hashValue2, submatches, offset %
            // decoQantum);
            ...
            while (position < decoQantum) {
              //		                    if (debug)
              //							printf("Old
              // hash %u [%d/%d]\n", windowHashes[position], position,
              // decoQantum);
            ...
            if (!inserted[smallestPosition]) {
              ...
              //                            {
              //                                printf("Inserted %d! ",
              // windowHashes[smallestPosition]);
              //                                print_singleSequence(sequence +
              // offset, wordLength);
              //                                printf("\n");
              //							}
              hashcounter_insert(hashCounter2, hashValue2);
...
void rsdb_spexClusterSequences(struct sequence *sequences,
  ...
  //    hashCounter = rsdb_winnowingSpex(sequences, numberOfSequences,
  // numberOfLetters);
  ...
  blockSequenceLists =
      memBlocks_initialize(sizeof(struct sequenceLists), 10000);
  ...
  sequencesLists = (struct sequenceLists **)global_malloc(
      sizeof(struct sequenceLists *) * numberOfSequences);
  ...
  while (hashtablePass < numHashtablePasses) {
      ...
      if (sequenceSize >= wordLength) {
        ...
        while (offset < sequenceSize - wordLength) {
          ...
          if (passhash % numHashtablePasses == hashtablePass) {
            ...
            word = sequence + offset;
            hashcounter_hash(hashCounter, word, wordLength, hashValue,
                             position);
    ...
    while (key >= 0) {
      ...
      if (finalLists[key].numEntries > maxListLength) {
        ...
        numWordLocations =
            postings_decodeList(finalLists[key], longListWordLocations);
        ...
        while (numWordLocations > 0) {
          ...
          numWordLocations--;
          sequenceLists =
              (struct sequenceLists *)memBlocks_newEntry(blockSequenceLists);
          sequenceLists->next = sequencesLists
              [longListWordLocations[numWordLocations].sequenceNumber];
  ...
  printf(
      "Identifying high-scoring sequence pairs / hierarchical clustering...");
  fflush(stdout);
  time = -clock();
    ...
    if (seq1->parent == NULL) {
      ...
      while (sequenceLists != NULL) {
        ...
        while (location2 < numWordLocations) {
          if (sequences[wordLocations[location2].sequenceNumber].parent ==
              NULL) {
            // Update accumulators
      ...
      while ((diagonalMatch = memSingleBlock_getCurrent(sequenceDiagonals)) !=
        ...
        /*
                        ...
                        {
                            // Calculate rough score based on number of matches,
        length of overlap region
                            score = 100 * diagonalMatch->numMatches *
        decoWordLength /
                                    minimum(seq1->length, seq2->length);

        //                    score = rsdb_score(seq1, seq2, relativeOffset,
        wildcardsThreshold);
        //                    if (score > 0)
                                printf("%d,%d matches=%d w=%d q=%d i=%d
        seqLength=%d\n",
                                       seq1->number, seq2->number,
        diagonalMatch->numMatches, decoWordLength,
                                       decoQantum, numIterations,
        minimum(seq1->length, seq2->length), score);
                                        }
        */
        ...
        //                if (score >= percentMatchesRequired)
        {
          ...
          // Case 2 - sequence1 has a parent, sequence2 does not
            ...
            score = rsdb_score(parent->children[0], seq2, relativeOffset,
                               wildcardsThreshold);
...
void rsdb_updateDiagonalMatches(struct memSingleBlock *diagonals,
  ...
  if (location1.sequenceNumber != location2.sequenceNumber) {
    ...
    // Otherwise create a new entry, ONLY if neither sequence1 or 2 belong to a
    // cluster
    if (!matchFound) // &&
...
uint4 rsdb_score(struct sequence *seq1, struct sequence *seq2,
  ...
  score = identityAlign_score(sequence2, length2, sequence1, length1,
                              -relativeOffset, targetScore);
