...
uint4 *wildcards_wildPositions, *wildcards_wildCount, wildcards_numWilds,
    wildcards_numWildCodes;

struct clusterWildcard
wildcards_clusterWildcards[wildcards_numClusterWildcards] = {
  { "LVIFM", 82961, { 3,  -1, -3, -2, 3, -2, -1, -2, -3, -2, 3,  -2,
                      -3, -1, 2,  0,  4, -2, -1, -1, -3, -2, -1, -4 },
    -0.443727 },
  { "GEKRQH", 141476, { -2, -1, 1,  0,  -2, 3, -1, 4,  0, -1, -3, 3,
                        0,  3,  -3, -1, -1, 3, -3, -2, 0, 2,  -1, -4 },
    -0.069308 },
  { "AVTIX", 4195410, { 1,  2,  -1, 0,  3, -1, 3,  -1, -2, -1, 3,  -2,
                        -2, -1, -1, -1, 0, -2, -1, -3, -2, -1, -1, -4 },
    -0.062484 },
  { "SETKDN", 4584, { -2, 0, 0,  2,  -2, 2, 2,  2,  4, -1, -2, 0,
                      4,  1, -3, -2, -1, 0, -2, -3, 3, 1,  -1, -4 },
    0.170094 },
  { "LVTPRFYMHCW", 1034833, { 1,  -1, -2, -1, 1, -1, 1, -1, -2, 2,  0,  1,
                              -1, 0,  2,  3,  2, 2,  2, 3,  -2, -1, -1, -4 },
    0.233131 },
  { "AGSDPH", 131854, { -2, 3, 3,  3,  -1, 0, 0,  0,  2, 2, -2, -1,
                        1,  0, -2, -2, -1, 1, -1, -3, 1, 0, 0,  -4 },
    0.311481 },
  { "LAGSVETKDPIRNQFYMHCWBZXU",
    16777215, { 1, 1, 1, 2, 1, 2, 1, 1, 1, 1, 1,  1,
...
float wildcards_scoreCandidates(struct wild *originalWildCandidates,
                                uint4 numWildCandidates, struct wild *wilds,
                                uint4 numWilds, float defaultWildscore) {
  ...
  wildCandidates =
      (struct wild *)global_malloc(sizeof(struct wild) * numWildCandidates);
  memcpy(wildCandidates, originalWildCandidates,
         sizeof(struct wild) * numWildCandidates);

  // Sort wild candidates
  //	qsort(wildCandidates, numWildCandidates, sizeof(struct wild),
  // wildcards_compareWilds);
  ...
  while (candidateNum < numWildCandidates) {
    ...
    wildSubset = wildcards_getSubset(wildCandidate, wilds, numWilds,
                                     &sizeWildSubset, &numOccurences);

    // Remove already process wildcards from subset
    wildcards_removeSubset(wildSubset, &sizeWildSubset, pastSubsets,
                           sizePastSubsets, &numOccurences);

    //        printf("wildSubset=%d\n", sizeWildSubset); fflush(stdout);
    score = wildcards_averageResidueWildMatch(wildCandidate, wildSubset,
                                              sizeWildSubset);
    //        printf("Average score=%f occurences=%d defScore=%f\n", score,
    // numOccurences, defaultWildscore);
    //        printf("Total saving: %f\n", (defaultWildscore - score) *
    // numOccurences);
    ...
    memcpy(wildcards_clusterWildcards[candidateNum].scoreMatrixRow,
           wildcards_scoreMatrixRow, sizeof(int2) * encoding_numLetters);
...
int4 wildcards_compareClusterWildcards(const void *wild1, const void *wild2) {
...
void wildcards_removeSubset(struct wild *set1, uint4 *size1, struct wild *set2,
                            uint4 size2, uint4 *numOccurences) {
...
struct wild *wildcards_joinSubset(struct wild *set1, uint4 *size1,
                                  struct wild *set2, uint4 size2) {
  set1 = (struct wild *)global_realloc(set1,
                                       sizeof(struct wild) * (*size1 + size2));
...
struct wild *wildcards_getSubset(struct wild wildCandidate, struct wild *wilds,
                                 uint4 numWilds, uint4 *sizeWildSubset,
                                 uint4 *numOccurences) {
  ...
  *sizeWildSubset = 0;
  ...
  // Build subset
  wildSubset =
      (struct wild *)global_malloc(sizeof(struct wild) * (*sizeWildSubset));
  count = 0;
  *sizeWildSubset = 0;
  while (count < numWilds) {
    if ((wilds[count].code & wildCandidate.code) == wilds[count].code) {
...
float wildcards_averageResidueWildMatch(struct wild wildCandidate,
                                        struct wild *wilds, uint4 numWilds) {
  unsigned char code;
  ...
  while (code < encoding_numLetters) {
    ...
    score =
        wildcards_scoreResidueWildMatch(wildCandidate, wilds, numWilds, code);
...
// Calculate the average score for aligning residue 'code' with given wildcard
// candidate
float wildcards_scoreResidueWildMatch(struct wild wildCandidate,
  ...
  if (getbit(wildCandidate.code, code)) {
    averageScore =
        wildcards_scoringConstant * wildcards_scoreMatrix.matrix[code][code] +
...
void wildcards_initializeCountOccurences(uint4 longestSequenceLength) {
  ...
  wildcards_wildPositions =
      (uint4 *)global_malloc(sizeof(uint4) * longestSequenceLength);
  position = 0;
  ...
  wildcards_wildCount =
      (uint4 *)global_malloc(sizeof(uint4) * wildcards_numWildCodes);
  wildCode = 0;
...
void wildcards_countOccurences(struct child *children, uint4 numChildren,
...
struct wild *wildcards_getOccurences(uint4 *numWilds) {
  ...
  wilds =
      (struct wild *)global_malloc(sizeof(struct wild) * wildcards_numWilds);
  count = 0;
  wildCode = 0;
  while (wildCode < wildcards_numWildCodes) {
...
void wildcards_readWildcards(char *filename) {
  ...
  while (count < wildcards_numClusterWildcards) {
    ...
    while (code < encoding_aaStartWildcards) {
      fscanf(file, "%d",
             &(wildcards_clusterWildcards[count].scoreMatrixRow[code]));
      code++;
...
void wildcards_outputWildcards(char *filename) {
  ...
  //          sizeof(struct clusterWildcard),
  // wildcards_compareClusterWildcards);
