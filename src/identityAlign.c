...
// Perform banded smith-waterman dynamic programming on the given query and
// subject sequences
...
// length1 - maxMismatch
int4 identityAlign_score(unsigned char *seq1, int4 length1, unsigned char *seq2,
                         int4 length2, int4 relativeOffset, uint4 targetScore) {
  ...
  if (length2 >= identityAlign_rowSizes) {
    ...
    // Malloc new rows
    identityAlign_bestRow =
        (int4 *)global_malloc(sizeof(int4) * identityAlign_rowSizes);
  }
  ...
  while (position1 < end1) {
    ...
    //        identityAlign_print(bandStart - seq2, bandEnd - seq2, end2 -
    // seq2);
    //		if (dloc==72249)
    //			smithWatermanTraceback_print(identityAlign_bestRow,
    // subjectLength);
  }
...
void identityAlign_free() { free(identityAlign_bestRow); }
