...
// Prototypes
struct dpResults gappedScoring_dpBeforeSeed(unsigned char *subject,
                                            struct PSSMatrix PSSMatrix,
                                            struct coordinate seed,
                                            int4 dropoff);
struct dpResults gappedScoring_dpAfterSeed(unsigned char *subject,
                                           struct PSSMatrix PSSMatrix,
...
int4 gappedScoring_score(struct ungappedExtension *ungappedExtension,
  ...
  beforeDpResults =
      gappedScoring_dpBeforeSeed(subject, PSSMatrix, seed, dropoff);
...
struct dpResults gappedScoring_dpBeforeSeed(unsigned char *subject,
  ...
  if (seed.subjectOffset >= gappedScoring_rowSizes) {
    ...
    // Malloc new rows
    gappedScoring_bestRow =
        (int4 *)global_malloc(sizeof(int4) * gappedScoring_rowSizes);
    gappedScoring_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * gappedScoring_rowSizes);
  }
...
#ifdef VERBOSE
  if (parameters_verboseDloc == blast_dloc)
    gappedExtension_printBeforeRow(gappedScoring_bestRow, subject, rowDropoff,
                                   columnDropoff);
#endif
...
#ifdef VERBOSE
  if (parameters_verboseDloc == blast_dloc)
    gappedExtension_printAfterRow(gappedScoring_bestRow + 1, subject,
