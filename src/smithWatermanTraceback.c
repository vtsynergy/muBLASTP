...
// Prototypes
// struct trace smithWatermanTraceback_trace(struct dpResults dpResults, int4
// queryLength);
void smithWatermanTraceback_print(int4 *row, int4 subjectLength);
struct dpResults smithWatermanTraceback_dynamicProgramming(
    unsigned char *subject, struct PSSMatrix PSSMatrix, int4 subjectLength);
struct trace smithWatermanTraceback_trace(struct dpResults dpResults,
                                          int4 queryLength,
...
struct gappedExtension *smithWatermanTraceback_build(struct PSSMatrix PSSMatrix,
  ...
  // Only perform dynamic programming on region containing high-scoring
  // alignment
  ...
  dpResults = smithWatermanTraceback_dynamicProgramming(subject, PSSMatrix,
                                                        subjectSize);
  ...
  trace = smithWatermanTraceback_trace(dpResults, PSSMatrix.length, PSSMatrix,
                                       subject);
  ...
  gappedExtension->subjectEnd =
      dpResults.best.subjectOffset + start.subjectOffset;
...
// the matrix), performs the simple operation of finding the path from the
// highest scoring point back
...
struct trace smithWatermanTraceback_trace(struct dpResults dpResults,
  ...
  while (queryPosition >= 0 && subjectPosition >= 0) {
    ...
    // PSSMatrix.matrix[queryPosition][subject[subjectPosition]], traceCode,
    // state);
...
struct dpResults smithWatermanTraceback_dynamicProgramming(
  ...
  if (subjectLength >= smithWatermanTraceback_rowSizes) {
    ...
    // Malloc new rows
    smithWatermanTraceback_matchRow =
        (int4 *)global_malloc(sizeof(int4) * smithWatermanTraceback_rowSizes);
    smithWatermanTraceback_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * smithWatermanTraceback_rowSizes);
    smithWatermanTraceback_insertSrow =
        (int4 *)global_malloc(sizeof(int4) * smithWatermanTraceback_rowSizes);
  }

  // Declare array to store traceback information, a row for each subject value
  traceback = (unsigned char **)global_malloc(sizeof(unsigned char *) *
                                              (queryLength + 1));

  queryDistance = 0;
  while (queryDistance < queryLength + 1) {
    // A column for each query value in the traceback array
    traceback[queryDistance] = (unsigned char *)global_malloc(
        sizeof(unsigned char) * (subjectLength + 1));
  ...
  //	if (dloc==39755)
  //	smithWatermanTraceback_print(smithWatermanTraceback_matchRow,
  // subjectLength);
  ...
  while (queryPosition < queryEnd) {
    //		printf("%d/%d\n", queryPosition - PSSMatrix.matrix, queryEnd -
    ...
    *tracebackColumn = 3;
    ...
    while (subjectPosition < subjectEnd) {
      ...
      *matchRow = match;
      ...
      // Set new Iy value
      //			printf("[%d/%d]", subjectPosition - subject,
      // subjectEnd - subject); fflush(stdout);
