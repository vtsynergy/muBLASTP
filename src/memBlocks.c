...
//
// These routines provide functionality for declaring a collection of large
// blocks of memory
// for holding individual data entries and allowing access to the portions of
// memory for each
// entry individually. Once an entry has been created, it will not be moved (ie.
// realloced).
...
// The advantage of using these routines is that they provide better locality
// and
...
void *memBlocks_newEntry(struct memBlocks *memBlocks) {
  ...
  if (memBlocks->numEntries[memBlocks->numBlocks - 1] >=
      ...
      //fprintf(stderr, "Warning: memBlocks_newEntry, please increase blockSize: %d\n", memBlocks->blockSizes);
...
void *memBlocks_getLastEntry(struct memBlocks *memBlocks) {
  ...
  lastEntry = ((char *)(memBlocks->lastBlock)) +
              (memBlocks->numEntries[memBlocks->numBlocks - 1] - 1) *
                  memBlocks->entrySize;
...
void *memBlocks_getCurrent(struct memBlocks *memBlocks) {
  ...
  if (memBlocks->currentEntry >=
      memBlocks->numEntries[memBlocks->currentBlock]) {
