...
//
// Functions for processing and rescoring a collection of high-scoring
// alignments
...
void alignments_initialize() {
  ...
  alignments_goodAlignments = memSingleBlock_initialize(
      sizeof(struct finalAlignment), constants_initialAllocGoodAlignments);
  alignments_finalAlignments = memSingleBlock_initialize(
...
// Given an alignment with a list of gapped extensions, remove the lowest
// scoring of
...
// Returns true if extension2 is contained in the rectangular region defined by
// extension2
...
void alignments_unpruneRegion(struct alignment *alignment,
...
void alignments_pruneRegion(struct alignment *alignment,
                            struct ungappedExtension *ungappedExtension) {
  struct ungappedExtension *currentExtension;
...
// alignments
void alignments_addGoodAlignment(int4 highestNominalScore,
                                 struct alignment *alignment) {
...
// Compare the two alignments' highest scores. Return -1 if alignment1 <
// alignment2
...
void alignments_sortFinalAlignments() {
  qsort(alignments_finalAlignments->block,
        alignments_finalAlignments->numEntries, sizeof(struct finalAlignment),
        alignments_compareFinalAlignments);
}

// Select the region for this alignment and return the subject
unsigned char *
alignments_selectRegion(struct alignment *alignment,
                        struct ungappedExtension *ungappedExtension) {
  ...
  if (alignment->unpackRegions == NULL) {
    ...
    unpackRegion = unpack_selectRegion(alignment->unpackRegions,
                                       alignment->numUnpackRegions,
                                       ungappedExtension->seed.subjectOffset);

    //        printf("Select region start=%d end=%d/%d seed=%d\n",
    // unpackRegion->startOffset,
    //        unpackRegion->endOffset, alignment->subjectLength,
    // ungappedExtension->seed.subjectOffset); fflush(stdout);
...
void alignments_checkForJoin(struct alignment *alignment,
    ...
    if (extension2 != extension1 &&
      ...
      if ((queryDistance >= 0 || subjectDistance >= 0) &&
          minimum(abs(queryDistance), abs(subjectDistance)) +
              ...
              2 * statistics_gappedFinalNominalDropoff) {
...
int4 alignments_compareAlignmentDescriptionLocations(const void *alignment1,
                                                     const void *alignment2) {
  const struct finalAlignment *a1, *a2;
  ...
  if (a1->alignment->descriptionLocation > a2->alignment->descriptionLocation) {
    return 1;
  } else if (a1->alignment->descriptionLocation <
             a2->alignment->descriptionLocation) {
    return -1;
  } else {
    return 0;
  }
...
void alignments_getFinalAlignmentDescriptions() {
  ...
  while ((finalAlignment =
              memSingleBlock_getCurrent(alignments_finalAlignments)) != NULL) {
    finalAlignment->description = descriptions_getDescription(
...
void alignments_printFinalAlignments() {
  ...
  while (count < alignments_finalAlignments->numEntries) {
    ...
    printf("%3d) Score=%d Dloc=%d\n", count,
           finalAlignment->highestNominalScore, alignment->descriptionLocation);
    count++;
  }
}
...
void alignments_printGoodAlignments() {
  ...
  while (count < alignments_goodAlignments->numEntries) {
    ...
    printf("%3d) Score=%d Dloc=%d\n", count, goodAlignment->highestNominalScore,
    ...
    while (ungappedExtension != NULL) {
      if (ungappedExtension->status != ungappedExtension_DELETED) {
        ...
        // Semi-gapped scoring
          ...
          ungappedExtension->nominalScore = semiGappedScoring_score(
              ungappedExtension, PSSMatrix, alignment->subjectLength,
              alignment->subject, statistics_gappedNominalDropoff);
        ...
        // Regular gapped scoring
          ...
          ungappedExtension->nominalScore = gappedScoring_score(
              ungappedExtension, PSSMatrix, alignment->subjectLength,
              alignment->subject, statistics_gappedNominalDropoff);
...
void alignments_loadSubjectsIntoMemory(struct PSSMatrix PSSMatrix) {
  ...
  if (parameters_numDisplayAlignments > 0 &&
    ...
    minimumDynamicCutoff = goodAlignment->highestNominalScore /
                           parameters_semiGappedR2 * parameters_semiGappedR1;
  ...
  while (position < alignments_finalAlignments->numEntries) {
    finalAlignment =
        memSingleBlock_getEntry(alignments_finalAlignments, position);
    alignment = finalAlignment->alignment;
...
void
alignments_regularGappedAlignment(struct PSSMatrix PSSMatrix,
                                  struct ungappedExtension *ungappedExtension,
                                  struct alignment *alignment) {
  int4 newScore;
  unsigned char *subject;
  ...
  newScore = gappedScoring_score(ungappedExtension, PSSMatrix,
...
void alignments_findTopFinalAlignments(struct PSSMatrix PSSMatrix) {
  ...
  while (position < parameters_numDisplayAlignments) {
    ...
    while (ungappedExtension != NULL) {
      ...
      if (ungappedExtension->status != ungappedExtension_DELETED &&
        ...
        alignments_regularGappedAlignment(PSSMatrix, ungappedExtension,
                                          alignment);
  ...
  while (position < alignments_goodAlignments->numEntries) {
    ...
    {
      // Add to final alignments
      alignments_addFinalAlignment(goodAlignment->highestNominalScore,
                                   alignment);
      ...
      finalAlignment = memSingleBlock_getEntry(
          alignments_finalAlignments, parameters_numDisplayAlignments - 1);

      // Calculate dynamic cutoffs
      blast_dynamicGappedNominalCutoff = finalAlignment->highestNominalScore;
      blast_dynamicNominalR1cutoff =
          blast_dynamicGappedNominalCutoff * parameters_semiGappedR1;
    }
...
void alignments_findFinalAlignments(struct PSSMatrix PSSMatrix) {
  ...
  if (parameters_numDisplayAlignments > 0 &&
    ...
    goodAlignment = memSingleBlock_getEntry(
        alignments_goodAlignments, parameters_numDisplayAlignments - 1);
  ...
  while ((goodAlignment =
              memSingleBlock_getCurrent(alignments_goodAlignments)) != NULL) {
    alignment = goodAlignment->alignment;
...
void alignments_getTracebacks(struct PSSMatrix PSSMatrix) {
  ...
  while (alignmentCount < parameters_numDisplayAlignments &&
    ...
    finalAlignment =
        memSingleBlock_getEntry(alignments_finalAlignments, alignmentCount);
    alignment = finalAlignment->alignment;
    blast_dloc = alignment->descriptionLocation;
        ...
        while (ungappedExtension != NULL) {
          ...
          if (ungappedExtension->status != ungappedExtension_DELETED) {
            ...
            if (((uint8)PSSMatrix.length * (uint8)alignment->subjectLength <
              ...
              gappedExtension = fasterGappedExtension_build(
                  ungappedExtension, PSSMatrix, alignment->subjectLength,
            ...
            // Otherwise use slower but more memory-efficient gapped alignment
              ...
              gappedExtension = gappedExtension_build(
                  ungappedExtension, PSSMatrix, alignment->subjectLength,
