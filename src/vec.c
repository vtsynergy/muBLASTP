...
struct vec *vec_reinit(struct vec *v, unsigned long int initsize) {
  ...
  if (!v) {
    ...
    if ((v = malloc(sizeof(*v))) && (v->vector = malloc(initsize))) {
      v->size = initsize;
      v->pos = v->len = 0;
      v->err = 0;
    } else if (v) {
...
void vec_resetpos(struct vec *v) { v->pos = 0; }

void vec_reseterr(struct vec *v) { v->err = 0; }
...
unsigned long int vec_getvbyte(struct vec *v, unsigned long int *n) {
  unsigned long int ret = 1, count = 0, get = 0;
  ...
  while (v->pos < v->len) {
    if (((get = ((uint8_t)v->vector[v->pos++])) & 0x80) == 0x80) {
      ...
      *n |= ((get & 0x7f) << count);
      ...
      if (ret == ((sizeof(*n) + 1)) && (get < (1 << (sizeof(*n) + 1)))) {
...
int vec_err(const struct vec *v) { return v->err; }

int vec_eof(const struct vec *v) { return (v->pos == v->len); }

unsigned long int vec_position(const struct vec *v) { return v->pos; }

unsigned long int vec_length(const struct vec *v) { return v->len; }
...
unsigned int vec_write(struct vec *src, int dstfd) {
  unsigned int wlen, len = src->len - src->pos;
