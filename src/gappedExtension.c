...
// uses
// the BLAST dropoff technique. This code is slower but more memory efficient
// than
// fasterGappedExtension, and allows for cases where only part of the subject
// has been unpacked
...
// Prototypes
struct dpResults
gappedExtension_dpBeforeSeed(struct PSSMatrix PSSMatrix, int4 dropoff,
                             struct coordinate seed,
                             struct unpackRegion *unpackRegion);
struct dpResults gappedExtension_dpAfterSeed(struct PSSMatrix PSSMatrix,
                                             int4 dropoff,
                                             struct unpackRegion *unpackRegion,
                                             int4 subjectLength,
...
struct trace gappedExtension_traceBeforeSeed(struct dpResults beforeDpResults,
                                             struct coordinate seed);
struct trace gappedExtension_traceAfterSeed(struct dpResults beforeDpResults,
                                            int4 queryLength);
...
gappedExtension_dpBeforeSeed(struct PSSMatrix PSSMatrix, int4 dropoff,
  ...
  if (gappedExtension_matchRow == NULL) {
    ...
    // insert-query values
    gappedExtension_rowSizes =
        2 * statistics_gappedFinalNominalDropoff / parameters_extendGap;

    // Malloc new rows
    gappedExtension_matchRow =
        (int4 *)global_malloc(sizeof(int4) * gappedExtension_rowSizes);
    gappedExtension_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * gappedExtension_rowSizes);
    gappedExtension_insertSrow =
        (int4 *)global_malloc(sizeof(int4) * gappedExtension_rowSizes);
  }
  ...
  if (seed.queryOffset > gappedExtension_numRows) {
    ...
    gappedExtension_traceback = (unsigned char **)global_realloc(
        gappedExtension_traceback,
  ...
  if (gappedExtension_tracebackData == NULL) {
    ...
    gappedExtension_tracebackData =
        global_malloc(sizeof(char) * gappedExtension_tracebackAlloc);
  }

  tracebackDataPtr =
      gappedExtension_tracebackData + gappedExtension_tracebackAlloc - 1;
  ...
  insertQrow =
      gappedExtension_insertQrow + (rowOffset % gappedExtension_rowSizes);
  insertSrow =
      gappedExtension_insertSrow + (rowOffset % gappedExtension_rowSizes);
  ...
  while (queryPosition > PSSMatrix.matrix && rowDropoff < columnDropoff) {
    ...
    if (subjectPosition - minRowDropoff >=
      ...
      newTracebackData = (unsigned char *)global_malloc(
          sizeof(char) * gappedExtension_tracebackAlloc * 2);
    ...
    // Initialize traceback pointer for this row
    tracebackRow =
        gappedExtension_traceback + (queryPosition - PSSMatrix.matrix);
    *tracebackRow = tracebackDataPtr - (subjectPosition - subject);
    tracebackColumn = tracebackDataPtr;
    tracebackDataPtr -= (subjectPosition - minRowDropoff + 1);
    ...
    rowOffset = (subjectPosition - subject);
    matchRow =
        gappedExtension_matchRow + (rowOffset % gappedExtension_rowSizes);
    insertQrow =
        gappedExtension_insertQrow + (rowOffset % gappedExtension_rowSizes);
    insertSrow =
        gappedExtension_insertSrow + (rowOffset % gappedExtension_rowSizes);
    ...
    *insertSrow = constants_gappedExtensionDummyValue;
    ...
    //		print(gappedExtension_matchRow, subject, rowDropoff,
    // columnDropoff);
  ...
  *tracebackColumn = 0;
  ...
  while (queryPosition < queryEnd && rowDropoff > columnDropoff) {
    ...
    if (maxRowDropoff - subjectPosition >=
      ...
      // Move existing data to end of new block
      memcpy(newTracebackData, gappedExtension_tracebackData,
             gappedExtension_tracebackAlloc);

      // Update traceback data pointer
      tracebackDataPtr =
          newTracebackData + (tracebackDataPtr - gappedExtension_tracebackData);

      // Update existing rows to point to new data block
      while (tracebackRow > gappedExtension_traceback) {
        tracebackRow--;
        *tracebackRow =
            newTracebackData + (*tracebackRow - gappedExtension_tracebackData);
      }
    ...
    *insertSrow = constants_gappedExtensionDummyValue;
    ...
    //        if (dloc==88197331)
    //			gappedExtension_printAfterRow(gappedExtension_matchRow,
    //subject,
    // rowDropoff,
    //                                          columnDropoff);
  }
...
// Debugging routine
void gappedExtension_printBeforeRow(int4 *row, unsigned char *subject,
...
// Debugging routine
void gappedExtension_printAfterRow(int4 *row, unsigned char *subject,
                                   unsigned char *rowDropoff,
                                   unsigned char *columnDropoff) {
  // TODO: Adjust to work with wrap-around
  unsigned char *subjectPosition = subject;
