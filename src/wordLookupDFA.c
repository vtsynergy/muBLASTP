...
struct aaFrequencyGroup *
wordLookupDFA_calcFrequencyGroups(struct memSingleBlock **groupsHash,
                                  struct PSSMatrix PSSMatrix, int4 numCodes,
                                  int4 wordLength);
...
void wordLookupDFA_build(struct PSSMatrix PSSMatrix, int4 numCodes,
                         int4 wordLength) {
  ...
  initialWords = (struct initialWord *)global_malloc(
      sizeof(struct initialWord) * numWords);
  ...
  while (queryPosition < PSSMatrix.length - wordLength + 1) {
    ...
    while (numNeighbours > 0) {
      ...
      if (initialWord->numQueryPositions > initialWord->allocQueryPositions) {
        ...
        initialWord->queryPositions = (int2 *)global_realloc(
            initialWord->queryPositions,
  ...
  groupsHash = (struct memSingleBlock **)global_malloc(
      sizeof(struct memSingleBlock *) * PSSMatrix.length);

  // Initialize each array cell to an empty list
  queryPosition = 0;
  while (queryPosition < PSSMatrix.length) {
    groupsHash[queryPosition] =
        memSingleBlock_initialize(sizeof(struct memSingleBlock *), 10);
    queryPosition++;
  }
  ...
  while (groupCodeword < numGroups) {
    ...
    *((struct memSingleBlock **)memSingleBlock_newEntry(
         groupsHash[queryPosition])) = groupList;
    groupCodeword++;
  }
      ...
      while (count2 < groupsHash[queryPosition]->numEntries) {
        ...
        if (groupLists[count]->numEntries > 0 &&
          ...
          while (code < numCodes) {
            ...
            while (queryPositionCount <
                   initialWords[codeword].numQueryPositions) {
          ...
          if (match) {
            ...
            *((int4 *)memSingleBlock_newEntry(groupLists[count])) =
                groupList2[0];
  ...
  // the full word length and then arrange them so the most frequent have
  // central positions
  aaFrequencyGroups = wordLookupDFA_calcFrequencyGroups(
      groupsHash, PSSMatrix, numCodes, wordLength - 1);

  // Declare memory for addresses of group sized blocks
  wordLookupDFA_blockAddresses =
      (unsigned char **)global_malloc(sizeof(unsigned char *) * numGroups);

  // Declare memory for final words
  wordLookupDFA = (unsigned char *)global_malloc(
      sizeof(char) * numWords + sizeof(int2) * numExternalPositions +
  ...
  // Stage 2 - Construct the more compact, final wordLookupDFA structure that is
  // arranged
  ...
  while (count < wordLookupDFA_numBlocks) {
    ...
    while (qPosList_numQPosLists > 0) {
      ...
      if (endExternalPositions - startExternalPositions +
              queryPositionList->numEntries + 1 >=
          256 - 3 * numCodes) {
        ...
        while (count2 > 0) {
          count2--;
          queryPositionEntry =
              memSingleBlock_getEntry(queryPositionList, count2);
          ...
          while (queryPositionEntry->codewords != NULL) {
            ...
            *endExternalPositions =
                wordLookupDFA_numAdditionalQueryPositions / constants_max_int2;
            endExternalPositions++;
            *endExternalPositions =
                wordLookupDFA_numAdditionalQueryPositions % constants_max_int2;
            endExternalPositions++;
    ...
    while (code < numCodes) {
      ...
      // At least one query position
        ...
        *currentPosition = (unsigned char)(((char *)endExternalPositions -
                                            (char *)externalPositions[code]) /
                                           sizeof(int2));
      }
  ...
  wordLookupDFA_groups =
      (struct group *)global_malloc(sizeof(struct group) * numGroups);
...
void wordLookupDFA_findNeighbours(struct PSSMatrix PSSMatrix,
      ...
      while (tempValue < wordLookupDFA_numCodes) {
        ...
        if (tempValue != oldValue) {
          ...
          if (currentScore - oldScore + newScore >= parameters_T) {
            ...
            newCodeword =
                wordLookupDFA_getCodeword(codes, wordLookupDFA_wordLength);
...
void wordLookupDFA_getNeighbours(struct PSSMatrix PSSMatrix, int4 queryPosition,
  ...
  if (!containsWild || score >= parameters_T) {
    ...
    codeword = wordLookupDFA_getCodeword(
        PSSMatrix.bestMatchCodes + queryPosition, wordLookupDFA_wordLength);
    ...
    wordLookupDFA_findNeighbours(PSSMatrix, queryPosition, numNeighbours,
                                 neighbours);
  }
}
...
// Given a total number of codes, and a word length, find all groups of codes
// that
// are wordLength in length and arrange them according to their background
// frequency
struct aaFrequencyGroup *
wordLookupDFA_calcFrequencyGroups(struct memSingleBlock **groupsHash,
                                  struct PSSMatrix PSSMatrix, int4 numCodes,
  ...
  aaFrequencyGroups = (struct aaFrequencyGroup *)global_malloc(
      sizeof(struct aaFrequencyGroup) * numGroups);
  arrangedGroups = (struct aaFrequencyGroup *)global_malloc(
      sizeof(struct aaFrequencyGroup) * numGroups);
  ...
  // Arrange into second array so that high frequency pairs have a central
  // location
      ...
      if (encoding_alphabetType == encoding_protein || word != 0) {
        ...
        if (word == 0) {
          ...
          if (!*queryPositions) {
            ...
            queryPositions =
                wordLookupDFA_additionalQueryPositions + *(queryPositions + 1);
          }
