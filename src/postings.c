...
void postings_addPosting(struct postingsList *postingsList,
                         uint4 sequenceNumber, uint4 offset);
// 32-bit hashing function
uint4 postings_hash32bit(char *sequence, uint4 sequenceLength, uint4 tableSize,
                         uint4 seed);
...
void postings_initialize() {
  ...
  postings_hashtable = (struct postingsList **)global_malloc(
      sizeof(struct postingsList *) * postings_hashtableSize);
  postings_seed = rand();
...
void postings_addEntry(unsigned char *word, uint4 wordLength,
                       uint4 sequenceNumber, uint4 offset) {
  ...
  hash32bit = postings_hash32bit(word, wordLength, postings_hashtableSize,
                                 postings_seed);
  ...
  // Create a new entry
  postingsList =
      (struct postingsList *)global_malloc(sizeof(struct postingsList));
...
struct finalList *postings_getSortedLists() {
  ...
  finalLists = (struct finalList *)global_malloc(sizeof(struct finalList) *
                                                 postings_numLists);
  ...
  qsort(finalLists, postings_numLists, sizeof(struct finalList),
        postings_compareLists);

  // Remove lists with less than 2 entries
  while (postings_numLists > 0 &&
         finalLists[postings_numLists - 1].numEntries < 2) {
    postings_numLists--;
...
uint4 postings_decodeList(struct finalList finalList,
  ...
  while (entry < numEntries) {
    ...
    //        printf("%d,%d (%d/%d entries)\n", sequenceNumber, offset, entry,
    // numEntries); fflush(stdout);
...
void postings_addPosting(struct postingsList *postingsList,
  ...
  if (postingsList->list == NULL) {
    ...
    if (encodedLength + postingsList->length > postingsList->malloced) {
      ...
      postingsList->list =
          global_realloc(postingsList->list, postingsList->malloced);
      postings_size += postingsList->malloced;
    }

    // Append data
    memcpy(postingsList->list + postingsList->length, postings_vbyteEncoded,
           encodedLength);
    postingsList->length += encodedLength;
  }
...
void postings_print() {
  printf("%d lists. Postings size=%.2f Mb\n", postings_numLists,
         (float)postings_size / 1024.0 / 1024.0);
}
...
uint8 postings_hash64bit(char *sequence, uint4 sequenceLength, uint4 seed) {
  ...
  for (c = 0; c < sequenceLength; c++)
    hashval ^=
        ((hashval << 5) + sequence[c] + (hashval >> 2) + (hashval << 11));
