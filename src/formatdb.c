...
int4 main(int argc, char *argv[]) {
  ...
  while (readFasta_readSequence()) {
    ...
    if (alphabetType == encoding_nucleotide) {
      ...
      totalWilds +=
          encoding_replaceWildcards(wildcardEdits, sequence, sequenceLength);

      // Declare memory to hold wildcard data
      startWildcardData = global_realloc(
          startWildcardData, sizeof(char) * wildcardEdits->numEntries * 5);
      ...
      while ((wildcardEdit = memSingleBlock_getCurrent(wildcardEdits)) !=
             NULL) {
    ...
    //        printf("[%s](%d)", readFasta_descriptionBuffer,
    // readFasta_descriptionLength); fflush(stdout);
...
uint4 determineDbAlphabetType(char *filename) {
  ...
  while (readFasta_readSequence() && sequenceCount < 10) {
    ...
    if (encoding_determineAlphabetType(sequence, sequenceLength) ==
        encoding_protein) {
