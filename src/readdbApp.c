...
//
// Debugging program that reads .sequences and .descriptions files and prints
// their contents
    ...
    while (readdb_readSequence(&sequence, &sequenceLength, &descriptionStart,
      ...
      if (encoding_alphabetType == encoding_protein &&
        ...
        if (numChildren + 1 > numClusterSizes) {
          clusterSizes =
              global_realloc(clusterSizes, sizeof(uint4) * (numChildren + 1));
        ...
        description =
            descriptions_getDescription(descriptionStart, descriptionLength);
        if (display) {
          //                	printf(">%s (dloc = %d)\n", description,
          // descriptionStart);
          printf(">%s\n", description);
