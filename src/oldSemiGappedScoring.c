...
// Prototypes
struct dpResults oldSemiGappedScoring_dpBeforeSeed(unsigned char *subject,
                                                   struct PSSMatrix PSSMatrix,
                                                   struct coordinate seed,
                                                   int4 dropoff);
struct dpResults oldSemiGappedScoring_dpAfterSeed(unsigned char *subject,
                                                  struct PSSMatrix PSSMatrix,
                                                  int4 dropoff,
                                                  int4 subjectLength);
...
int4 oldSemiGappedScoring_score(struct ungappedExtension *ungappedExtension,
  ...
  beforeDpResults = oldSemiGappedScoring_dpBeforeSeed(
  ...
  afterDpResults = oldSemiGappedScoring_dpAfterSeed(
...
struct dpResults oldSemiGappedScoring_dpBeforeSeed(unsigned char *subject,
  ...
  if (seed.subjectOffset >= oldSemiGappedScoring_rowSizes) {
    ...
    // Malloc new rows
    oldSemiGappedScoring_bestRow =
        (int4 *)global_malloc(sizeof(int4) * oldSemiGappedScoring_rowSizes);
    oldSemiGappedScoring_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * oldSemiGappedScoring_rowSizes);
  }
  ...
  *insertQrow = insertS = match - parameters_semiGappedOpenGap;
  ...
  while (queryPosition > PSSMatrix.matrix && rowDropoff < columnDropoff) {
    ...
    //        if (dloc == 746829265)
    //                print(oldSemiGappedScoring_bestRow, subject, rowDropoff,
    // columnDropoff);
  }
...
struct dpResults oldSemiGappedScoring_dpAfterSeed(unsigned char *subject,
  ...
  *insertQrow = insertS = match - parameters_semiGappedOpenGap;
  ...
  //    if (dloc == 88197331)
  //        print2(oldSemiGappedScoring_bestRow + 1, subject, rowDropoff,
  // columnDropoff);
