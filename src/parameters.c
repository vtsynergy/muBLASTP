...
// DB_INDEX
int parameters_num_threads = 1;
//int parameters_batch_size = BATCH_SIZE;
...
char parameters_filterEnabled = 0;
...
#define NUM_ARGUMENTS 25
  struct argument parameters_arguments[NUM_ARGUMENTS] = {
    { 'd', parameters_STRING, &parameters_subjectDatabaseFile, "Database", "" },
    { 'i', parameters_STRING, &parameters_queryFile, "Query File", "" },
    { 'A',                                        parameters_INT, &parameters_A,
      "Multiple Hits window size (protein only)", "40" },
    { 'f',           parameters_INT,
      &parameters_T, "Threshold for extending hits (protein only)",
      "11" },
    ...
    { 'y',
      parameters_FLOAT,
      &parameters_ungappedNormalizedDropoff,
      "Dropoff (X) for blast extensions in bits",
      "7.0 for protein, 20.0 for nucleotide" },
    { 'P',                       parameters_BYTE,
      &parameters_oneHitTrigger, "0 for multiple hit, 1 for single hit",
      "0" },
    { 'F',
      parameters_BYTE,
      &parameters_filterEnabled,
      "Filter query sequence using DUST/SEG (1=True or 0=False)",
      "1" },
    { 'G',
      parameters_SHORTINT,
      &parameters_startGap,
      "Cost to open a gap (Matrix dependant)",
      "11 for protein, 5 for nucleotide" },
    { 'E',
      parameters_SHORTINT,
      &parameters_extendGap,
      "Cost to extend a gap (Matrix dependant)",
      "1 for protein, 2 for nucleotide" },
    { 'O',
      parameters_SHORTINT,
      &parameters_semiGappedStartGap,
      "Cost to open a gap (Semi-gapped alignment, Matrix dependant)",
      "7" },
    { 'X',
      parameters_FLOAT,
      &parameters_gappedNormalizedDropoff,
      "X dropoff value for gapped alignment (in bits)",
      "15.0 for protein, 30.0 for nucleotide" },
    { 'N',
      parameters_FLOAT,
      &parameters_ungappedNormalizedTrigger,
      "Number of bits to trigger gapping",
      "22.0 for protein, 25.0 for nucleotide" },
    { 'Z',
      parameters_FLOAT,
    ...
    { 'M', parameters_STRING, &parameters_scoringMatrix, "Matrix", "BLOSUM62" },
    { 'v',
      parameters_INT,
    ...
    { 'b',
      parameters_INT,
      &parameters_numDisplayTracebacks,
      "Number of database sequences to show alignments for (B)",
      "250" },
    { 'W',         parameters_BYTE,                   &parameters_wordSize,
      "Word size", "3 for protein, 11 for nucleotide" },
    { 'z',
      parameters_INT,
      &parameters_databaseSize,
      "Effective length of the database (use zero for the real size)",
      "0" },
    { 'q',                       parameters_INT,
      &parameters_mismatchScore, "Penalty for a nucleotide mismatch",
      "-3" },
    ...
    { 'S', parameters_BYTE, &parameters_strands,
      ...
      "both, 1 is top, 2 is bottom",
      "3" },
    { 'a',
      parameters_BYTE,
      &parameters_allClusterMembers,
      "Display alignments for all members of a cluster  (1=True or 0=False)",
      "1" },
    { 't', parameters_INT, &parameters_num_threads, "Number of threads", "1" },
    //{ 'B',                    parameters_INT,
      //&parameters_batch_size, "Number of queries in a query batch",
      //"1000" },
    /*   {'I', parameters_BYTE, &parameters_useIndex,
         ...
         {'s', parameters_BYTE, &parameters_scoringSystem,
          "Scoring System (0 = Bytepacked + Semi + RI  1 = RI only  2 = Semi
       only  3 = Original  4 = Semi + RI  5 = Table + RI)", "0"},
         {'R', parameters_FLOAT, &parameters_semiGappedR1,
          "Ratio between semi-gapped/byte-packed and gapped cutoffs", "0.68 for
       semi-gapped, 0.85 for byte-packed"},
         ...
         {'V', parameters_BYTE, &parameters_overlap,
          "Maximum distance between two hits for them to be regarded as
       overlapping", "3"},
  ...
  if (parameters_subjectDatabaseFile[0] == '\0') {
    ...
    mpi_parameters_subjectDatabaseFile = (char*)global_malloc(strlen(parameters_subjectDatabaseFile) + 20);
    sprintf(mpi_parameters_subjectDatabaseFile, "%s.rank%d", parameters_subjectDatabaseFile, rank);
...
void parameters_loadDefaults(unsigned char alphabetType) {
  ...
  if (alphabetType == encoding_nucleotide) {
    ...
    parameters_setDefault(parameters_bytepackExtendGap,
                          defaultPenalties.extendGap);
    parameters_bytepackOpenGap =
        parameters_bytepackStartGap + parameters_bytepackExtendGap;

    parameters_bytepackExtend4Gap = 4 * parameters_bytepackExtendGap;
    parameters_bytepackOpen4Gap =
        parameters_bytepackStartGap + parameters_bytepackExtend4Gap;
  ...
  // Invert sign of gap existance penalty and extend gap penalty if either is
  // negative
  ...
  // If no semi-gapped open gap value given at the command line, use default
  // difference
  parameters_setDefault(parameters_semiGappedStartGap,
                        parameters_startGap -
                            defaultPenalties.semiGappedDifference);
...
void parameters_findScoringMatrix() {
  ...
  ncbircFilename =
      (char *)global_malloc(sizeof(char) * (strlen(homeDirectory) + 9));
  sprintf(ncbircFilename, "%s/.ncbirc", homeDirectory);

  // Check for existence of NCBI file
  if ((ncbircFile = fopen(ncbircFilename, "r")) != NULL) {
    // Determine the location of the scoring matrix file by consulting the
    // .ncbirc file
    ...
    if (!(fscanf(ncbircFile, "[NCBI]\nData=%s",
      ...
      fprintf(stderr, "Error reading scoring matrix path from %s file\n",
              ncbircFilename);
      fprintf(stderr,
              "BLAST requires the file .ncbirc in the user's home directory\n");
      fprintf(stderr, "containing the text:\n\n");
      fprintf(stderr, "[NCBI]\n");
      fprintf(stderr, "Data=/home/user/fsa-blast/data\n\n");
      fprintf(stderr, "Where the path specified contains scoring matrix files "
                      "(ie. BLOSUM62)\n");
      exit(-1);
    }
    fclose(ncbircFile);
  ...
  sprintf(parameters_scoringMatrixPath, "data/%s", 
...
static const struct defaultPenalties parameters_defaultPenalties
    [NUM_DEFAULT_PENALTIES] = { { "BLOSUM45", 14, 2, 4 },
                                { "BLOSUM50", 13, 2, 4 },
                                { "BLOSUM62", 11, 1, 4 },
                                { "BLOSUM80", 10, 1, 5 },
                                { "PAM30", 10, 1, 7 },
...
struct defaultPenalties parameters_getDefaultPenalties(char *scoringMatrix) {
  ...
  while (count < NUM_DEFAULT_PENALTIES) {
    ...
    if (strcmp(parameters_defaultPenalties[count].scoringMatrix,
               scoringMatrix) == 0) {
