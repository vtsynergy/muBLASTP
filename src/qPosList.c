...
void qPosList_processList(int2 *queryPositions, int2 numQueryPositions,
                          int4 codeword);
...
void qPosList_initialize(int4 maxNumLists) {
  ...
  qPosList_qPosLists = (struct memSingleBlock *)global_malloc(
      sizeof(struct memSingleBlock) * maxNumLists);
  ...
  qPosList_initialQPosLists = (struct initialQPosList *)global_malloc(
      sizeof(struct initialQPosList) * maxNumLists);
...
void qPosList_addList(int2 *queryPositions, int2 numQueryPositions,
                      int4 codeword) {
  qPosList_initialQPosLists[qPosList_numInitialQPosLists].queryPositions =
      queryPositions;
  qPosList_initialQPosLists[qPosList_numInitialQPosLists].numQueryPositions =
      numQueryPositions;
...
void qPosList_processLists() {
  ...
  while (qPosList_numInitialQPosLists > 0) {
    ...
    queryPositions =
        qPosList_initialQPosLists[qPosList_numInitialQPosLists].queryPositions;
    numQueryPositions = qPosList_initialQPosLists[qPosList_numInitialQPosLists]
                            .numQueryPositions;
...
void qPosList_processList(int2 *queryPositions, int2 numQueryPositions,
  ...
  // Iterative through existing query positions lists (ordered from longest to
  // shortest)
  while (listCount < qPosList_numQPosLists) {
    ...
    while ((queryPosition = memSingleBlock_getCurrent(list)) != NULL &&
           subset) {
      ...
      while (queryPositionCount < numQueryPositions) {
        ...
        if (queryPosition->queryPosition ==
            queryPositions[queryPositionCount]) {
          break;
        }
        // The query position is not present in the new list, then existing list
        // is not a subset of the new one
        else if (queryPosition->queryPosition <
                 queryPositions[queryPositionCount]) {
          subset = 0;
          break;
        }
      ...
      // If the query positions in the existing list processed so far match all
      // of
    ...
    if (subset) {
      // If this existing list is a subset of the new list then add the
      // new/additional
      ...
      while (numQueryPositions > 0) {
        ...
        while ((queryPosition = memSingleBlock_getCurrent(list)) != NULL &&
          ...
          // Found it
          if (queryPosition->queryPosition ==
              queryPositions[numQueryPositions]) {
            present = 1;
            break;
          }
        }
...
// Compare the two query position list lengths. Return 1 if list1 length > list2
// length
