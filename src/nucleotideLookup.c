...
void nucleotideLookup_build(struct PSSMatrix PSSMatrix, int4 packedWordLength) {
  ...
  initialLookup = (struct initialWord *)global_malloc(
      sizeof(struct initialWord) * numEntries);
  ...
  while (queryPosition < PSSMatrix.length - wordLength + 1) {
    ...
    if (!(queryPosition < PSSMatrix.strandLength &&
      ...
      while (byteNumber < packedWordLength) {
        ...
        codeword = codeword << 8;
        codeword |= (int4)encoding_bytePack(PSSMatrix.queryCodes +
                                            queryPosition + byteNumber * 4);
        byteNumber++;
      }
      ...
      if (initialWord->numQueryPositions > initialWord->allocQueryPositions) {
        ...
        initialWord->queryPositions = (uint4 *)global_realloc(
  ...
  if (numAdditionalPositions > constants_max_int2 ||
      PSSMatrix.length > constants_max_int2) {
    ...
    nucleotideLookup_additionalPositions_large =
        (uint4 *)global_malloc(sizeof(uint4) * numAdditionalPositions);
  }
  // Shorter queries we use the smaller 16-bit lookup table
    ...
    // Declare memory for additional query positions
    nucleotideLookup_additionalPositions =
        (uint2 *)global_malloc(sizeof(uint2) * numAdditionalPositions);
  }

  // Declare memory for bit-lookup table
  numBitWords = numEntries / 32;
  nucleotideLookup_bitLookup =
      (uint4 *)global_malloc(sizeof(int4) * numBitWords);
  ...
  while (codeword < numEntries) {
    ...
    if (initialWord->numQueryPositions > 0) {
      nucleotideLookup_bitLookup[codeword >> 5] |=
    ...
    // Multiple query positions
      ...
      if (nucleotideLookup_largeTable) {
        nucleotideLookup_large[codeword] =
        ...
        // Add null terminator
        nucleotideLookup_additionalPositions_large
            [nucleotideLookup_numAdditionalPositions] = 0;
        nucleotideLookup_numAdditionalPositions++;
      } else {
