...
// Prototypes
struct coordinate ungappedExtension_findNucleotideSeed(
...
// Perform an ungapped extension when the seed is only a single hit on the
// diagonal, rather
// than a pair of hits.
struct ungappedExtension *
ungappedExtension_oneHitExtend(int2 **queryHit, unsigned char *subjectHit,
                               struct PSSMatrix PSSMatrix,
                               unsigned char *subject) {
  int2 **queryPosition;
  unsigned char *subjectPosition, *subjectStart, *subjectEnd;
...
// Perform one-hit seeded ungapped extension for nucleotide, 1 packed-byte at a
// time
struct ungappedExtension *ungappedExtension_nucleotideExtend(
    int4 queryHitOffset, int4 subjectHitOffset, struct PSSMatrix PSSMatrix,
    unsigned char *subject, uint4 subjectLength) {
  ...
  // Determine minimum query position; either start of the query or start of the
  // second strand
  if (queryHitOffset <= PSSMatrix.strandLength) {
    if (queryHitOffset < subjectHitOffset * 4)
      ...
      minQueryPosition =
          PSSMatrix.bytePackedCodes + queryHitOffset - subjectHitOffset * 4;
  } else {
  ...
  queryPosition = PSSMatrix.bytePackedCodes + queryHitOffset -
                  parameters_wordTableLetters - 4;
  subjectPosition = subjectStart =
      subject + subjectHitOffset - parameters_wordTableBytes - 1;

  // Consider partial match of first byte before hit
  matchLettersScore =
      PSSMatrix_packedLeftMatchScores[*queryPosition ^ *subjectPosition];
  ungappedExtension_bestScore += matchLettersScore;
  changeSinceBest = -matchLettersScore;

  // Move back through alignment until start of query or subject, or until
  // dropoff
  ...
  // Determine maximum query position; either end of the query or end of the
  // first strand
  if (queryHitOffset <= PSSMatrix.strandLength) {
    if (PSSMatrix.strandLength - queryHitOffset <
        subjectLength - subjectHitOffset * 4)
      maxQueryPosition = PSSMatrix.bytePackedCodes + PSSMatrix.strandLength - 4;
    else
      maxQueryPosition = PSSMatrix.bytePackedCodes +
                         (subjectLength - subjectHitOffset * 4) +
                         queryHitOffset - 4;
  } else {
    if (PSSMatrix.length - queryHitOffset <
        subjectLength - subjectHitOffset * 4)
  ...
  matchLettersScore =
      PSSMatrix_packedRightMatchScores[*queryPosition ^ *subjectPosition];
  ungappedExtension_bestScore += matchLettersScore;
  changeSinceBest = -matchLettersScore;

  // Move forward through alignment until end of query or subject, or until
  // dropoff
  ...
  if (ungappedExtension_bestScore >= blast_ungappedNominalTrigger) {
    ...
    if (parameters_verboseDloc == blast_dloc) {
      ...
      printf("%d,%d - %d,%d\n", newUngappedExtension->start.queryOffset,
             newUngappedExtension->start.subjectOffset,
             newUngappedExtension->end.queryOffset,
             newUngappedExtension->end.subjectOffset);
      fflush(stdout);
      printf("seed=%d,%d\n", newUngappedExtension->seed.queryOffset,
             newUngappedExtension->seed.subjectOffset);
    }
#endif
...
// Check that the given coordinate pair is part of a hit of length at least
// hitLength
int ungappedExtension_checkHit(uint4 queryHitOffset, uint4 subjectHitOffset,
  ...
  while (queryPosition >= minQueryPosition) {
    ...
    //        printf("<%d< (%d)", match, queryPosition -
    // PSSMatrix.bytePackedCodes);
...
void ungappedExtension_findSeed(struct ungappedExtension *ungappedExtension,
  ...
  if (ungappedExtension->seed.queryOffset == -1 &&
    ...
    if (encoding_alphabetType == encoding_protein) {
      ungappedExtension->seed = ungappedExtension_findProteinSeed(
          ungappedExtension, PSSMatrix, subject);
    } else {
      ungappedExtension->seed = ungappedExtension_findNucleotideSeed(
          ungappedExtension, PSSMatrix, subject);
    }
  }
}

// Find the seed point (middle of the longest run of matching bytes) of an
// ungapped extension
...
struct coordinate ungappedExtension_findNucleotideSeed(
  ...
  queryPosition =
      PSSMatrix.bytePackedCodes + ungappedExtension->start.queryOffset;
  ...
  subjectMiddle = (ungappedExtension->start.subjectOffset +
  ...
  while (subjectPosition <= subjectEnd) {
    if (*subjectPosition == *queryPosition) {
      ...
      if (run > longestRun) {
        ...
        longestRun = run;
        subjectMiddle =
            ((subjectPosition - 1 - subject) + (subjectStartRun - subject)) / 2;
      }
  ...
  diagonal = ungappedExtension->start.subjectOffset -
...
// Find the seed point (the middle of the 11-aa long highest scoring region) of
// the
// given ungapped extension
struct coordinate
ungappedExtension_findProteinSeed(struct ungappedExtension *ungappedExtension,
                                  struct PSSMatrix PSSMatrix,
                                  unsigned char *subject) {
  ...
  if (ungappedExtension->end.queryOffset -
    ...
    queryWindowStart = queryWindowEnd =
        PSSMatrix.matrix + ungappedExtension->start.queryOffset;
    ...
    while (queryWindowEnd <
           PSSMatrix.matrix + ungappedExtension->end.queryOffset) {
...
struct ungappedExtension *
ungappedExtension_simpleScoring(unsigned char *sequence1, uint4 offset1,
                                unsigned char *sequence2, uint4 offset2) {
  ...
  while (position2 > 0 && position1 > 0 &&
         score + dropoff >= ungappedExtension_bestScore) {
  ...
  /*
      ...
      if (-ungappedExtension_bestScore > originalDropoff)
      {
          ...
          while (changeSinceBest > dropoff)
          {
                  ...
                  if (changeSinceBest > 0)
                  {
                          // Keep updating best score and resetting
     change-since-best
                          // whilst we are reading positive scores
                          do
                          {
     ...
     memBlocks_newEntry(ungappedExtension_extensions);
     ...
     ungappedExtension_findProteinSeed(newUngappedExtension, PSSMatrix,
     subject);
          // Initialize next to null
          newUngappedExtension->next = NULL;
          newUngappedExtension->nominalScore = ungappedExtension_bestScore;
...
inline void ungappedExtension_localExtend()
{
        ...
        unsigned char* subjectPosition, *newSubjectStart = NULL, *newSubjectEnd
= NULL;
...
void ungappedExtension_findBest(struct PSSMatrix PSSMatrix, unsigned char*
subject, int4 subjectLength)
{
    ...
    while (offset < subjectLength)
    {
        ...
        score = 0;
        while ((match = (*queryPosition)[*subjectPosition]) >
constants_sentinalScore)
                {
                // Accumulate scores
                        score += match;
...
// Returns a copy of the current ungappedExtension information bundled into a
structure
struct ungappedExtension* ungappedExtension_copy(struct PSSMatrix PSSMatrix,
unsigned char* subject)
{
        struct ungappedExtension* newUngappedExtension;
...
ungappedExtension_subjectEnd;
        // Determine offsets from pointers
        newUngappedExtension->start.queryOffset = ungappedExtension_queryStart -
PSSMatrix.matrix;
        newUngappedExtension->end.queryOffset = ungappedExtension_queryEnd -
PSSMatrix.matrix;
        newUngappedExtension->start.subjectOffset =
ungappedExtension_subjectStart - subject;
        newUngappedExtension->end.subjectOffset = ungappedExtension_subjectEnd -
subject;
        ...
        // Find the seed point
        newUngappedExtension->seed =
ungappedExtension_findSeed(newUngappedExtension, PSSMatrix, subject);
...
// Copy the list of "from" ungappedExtensions to "to" and clear their scores and
status in the
// process
struct ungappedExtension* ungappedExtension_duplicate(struct ungappedExtension*
from,
...
//        printf("Duplicate: %d,%d -> %d,%d\n", from->start.queryOffset,
from->start.subjectOffset,
//               from->end.queryOffset, from->end.subjectOffset);
fflush(stdout);
...
//        newUngappedExtension->offset = from->offset;
        ...
        newUngappedExtension->seed.queryPosition = from->seed.queryOffset +
PSSMatrix.matrix;
        newUngappedExtension->seed.subjectPosition = from->seed.subjectOffset +
subject;
...
// Returns a copy of an ungappedExtension constructed using given hspindex
information
struct ungappedExtension* ungappedExtension_construct(struct PSSMatrix
PSSMatrix,
                                     unsigned char* subject, int4*
hspInformation)
{
        struct ungappedExtension* newUngappedExtension;
        ...
        // Copy all of the query/subject start/end pointer information
        newUngappedExtension->start.queryOffset = readLittleEndianInt((unsigned
char*)hspInformation); hspInformation++;
        newUngappedExtension->start.subjectOffset =
readLittleEndianInt((unsigned char*)hspInformation); hspInformation++;
        newUngappedExtension->end.queryOffset = readLittleEndianInt((unsigned
char*)hspInformation); hspInformation++;
        newUngappedExtension->end.subjectOffset = readLittleEndianInt((unsigned
char*)hspInformation); hspInformation++;
...
//    	= newUngappedExtension->end.subjectOffset -
newUngappedExtension->end.queryOffset;
        newUngappedExtension->nominalScore = readLittleEndianInt((unsigned
char*)hspInformation); hspInformation++;
        newUngappedExtension->scoringStatus = ungappedExtension_UNGAPPED;
