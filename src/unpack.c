...
int4 unpack_compareUnpackRegions(const void *unpackRegion1,
                                 const void *unpackRegion2) {
...
// Return the unpack region in the given list that contains the given subject
// offset
struct unpackRegion *unpack_selectRegion(struct unpackRegion *unpackRegions,
    ...
    while (numUnpackRegions > 0) {
        if (unpackRegions->startOffset <= subjectOffset &&
                unpackRegions->endOffset > subjectOffset) {
...
// Returns a list of subject region start/end values that cover the area likely
// to be
// processed during gapped extension, or the maximum area to be processed if
// maximum=1
uint4 unpack_getRegions(struct PSSMatrix PSSMatrix, struct alignment *alignment,
                        char maximum, struct memBlocks *regions) {
  struct ungappedExtension *ungappedExtension;
  int4 regionStart, regionEnd, subjectOffset, queryOffset, queryBefore,
      queryAfter;
  int4 maxRegionStart, maxRegionEnd;
  uint4 maxNumRegions = 0;
  struct unpackRegion *firstRegion = NULL, *currentRegion, *lastRegion,
                      *nextRegion;
  ...
  while (ungappedExtension != NULL) {
    if (ungappedExtension->status != ungappedExtension_DELETED) {
      // Calculate the maximum possible region to be covered by this gapped
      // alignment
      ...
      maxRegionStart =
          subjectOffset - queryBefore - statistics_gappedFinalNominalDropoff -
          (queryBefore * PSSMatrix.highestValue / parameters_extendGap) - 2;
      if (maxRegionStart < 0)
        maxRegionStart = 0;

      maxRegionEnd =
          subjectOffset + queryAfter + statistics_gappedFinalNominalDropoff +
      ...
      if (maximum) {
        ...
        // Calculate start and end of region likely to be covered by this gapped
        // alignment
        regionStart = ungappedExtension->start.subjectOffset -
                      constants_unpackRegionExtend;
        if (regionStart < 0)
          regionStart = 0;

        regionEnd = ungappedExtension->end.subjectOffset +
                    constants_unpackRegionExtend + 1;
      ...
      if (currentRegion->startOffset < alignment->subjectLength) {
        ...
        while ((nextRegion < lastRegion) &&
          ...
          if (nextRegion->endOffset <= currentRegion->endOffset) {
            ...
            // This region partially overlaps with current region; extend
            // currentRegion
