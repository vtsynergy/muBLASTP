...
int4 *fasterGappedExtension_matchRow_multi[BATCH_SIZE] = { NULL };
int4 *fasterGappedExtension_insertQrow_multi[BATCH_SIZE] = { NULL };
int4 *fasterGappedExtension_insertSrow_multi[BATCH_SIZE] = { NULL };
int4 fasterGappedExtension_rowSizes_multi[BATCH_SIZE] = { 0 };
unsigned char **fasterGappedExtension_traceback_multi[BATCH_SIZE] = { NULL };
int4 fasterGappedExtension_numRows_multi[BATCH_SIZE] = { 0 };
int4 fasterGappedExtension_numColumns_multi[BATCH_SIZE] = { 0 };

// Prototypes
struct dpResults fasterGappedExtension_dpBeforeSeed_multi(
    unsigned char *subject, struct PSSMatrix PSSMatrix, struct coordinate seed,
    int4 dropoff, int queryNum);
struct dpResults fasterGappedExtension_dpAfterSeed_multi(
    unsigned char *subject, struct PSSMatrix PSSMatrix, int4 dropoff,
    int4 subjectLength, int queryNum);

// Build a gapped extension with a trace and nominal score from the seed point
// of an ungapped
// extension using dynamic programming
struct gappedExtension *fasterGappedExtension_build_multi(
  ...
  // seed.subjectOffset, PSSMatrix.length, subjectSize);

  beforeDpResults = fasterGappedExtension_dpBeforeSeed_multi(
      subject, PSSMatrix, seed, dropoff, queryNum);
  ...
  afterDpResults = fasterGappedExtension_dpAfterSeed_multi(
      choppedSubject, choppedPSSMatrix, dropoff, choppedSubjectSize, queryNum);

  // Trace back to get the trace for the seed onwards
  afterTrace = fasterGappedExtension_traceAfterSeed(afterDpResults,
                                                    choppedPSSMatrix.length);
  ...
  // at
  // the seed position
  gappedExtension->nominalScore = beforeDpResults.bestScore +
                                  afterDpResults.bestScore +
                                  choppedPSSMatrix.matrix[0][choppedSubject[0]];
...
struct dpResults fasterGappedExtension_dpBeforeSeed_multi(
  ...
  if (seed.subjectOffset >= fasterGappedExtension_rowSizes_multi[queryNum]) {
    // Free existing rows
    free(fasterGappedExtension_matchRow_multi[queryNum]);
    free(fasterGappedExtension_insertQrow_multi[queryNum]);
    free(fasterGappedExtension_insertSrow_multi[queryNum]);
    // Set size to double current needed length
    fasterGappedExtension_rowSizes_multi[queryNum] = (seed.subjectOffset) * 2;
    // Malloc new rows
    fasterGappedExtension_matchRow_multi[queryNum] = (int4 *)global_malloc(
        sizeof(int4) * fasterGappedExtension_rowSizes_multi[queryNum]);
    fasterGappedExtension_insertQrow_multi[queryNum] = (int4 *)global_malloc(
        sizeof(int4) * fasterGappedExtension_rowSizes_multi[queryNum]);
    fasterGappedExtension_insertSrow_multi[queryNum] = (int4 *)global_malloc(
        sizeof(int4) * fasterGappedExtension_rowSizes_multi[queryNum]);
  }
  ...
  // fasterGappedExtension_numColumns); fflush(stdout);

  // Increase number of columns in traceback array if neccessary
  if (seed.subjectOffset > fasterGappedExtension_numColumns_multi[queryNum]) {
    // For each existing row
    queryDistance = 0;
    while (queryDistance < fasterGappedExtension_numRows_multi[queryNum]) {
      // Increase number of columns
      fasterGappedExtension_traceback_multi[queryNum][queryDistance] =
    ...
    fasterGappedExtension_numColumns_multi[queryNum] = seed.subjectOffset;
  }

  // If more rows are required
  if (seed.queryOffset > fasterGappedExtension_numRows_multi[queryNum]) {
    // Increase number of row pointers
    fasterGappedExtension_traceback_multi[queryNum] =
        (unsigned char **)global_realloc(
            fasterGappedExtension_traceback_multi[queryNum],
            sizeof(unsigned char *) * (seed.queryOffset));

    // Declare new rows
    while (fasterGappedExtension_numRows_multi[queryNum] < seed.queryOffset) {
      fasterGappedExtension_traceback_multi
          [queryNum][fasterGappedExtension_numRows_multi[queryNum]] =
              (unsigned char *)global_malloc(
                  sizeof(unsigned char) *
                  (fasterGappedExtension_numColumns_multi[queryNum]));

      fasterGappedExtension_numRows_multi[queryNum]++;
    }
  }
  ...
  matchRow = fasterGappedExtension_matchRow_multi[queryNum] + rowOffset;
  insertQrow = fasterGappedExtension_insertQrow_multi[queryNum] + rowOffset;
  insertSrow = fasterGappedExtension_insertSrow_multi[queryNum] + rowOffset;
  ...
  tracebackRow = fasterGappedExtension_traceback_multi[queryNum] +
                 (queryPosition - PSSMatrix.matrix);
  tracebackColumn = *tracebackRow + (subjectPosition - subject);

  //    printf("[%d,%d]\n", seed.subjectOffset, (subjectPosition - subject));
  // fflush(stdout);
  //    printf("[%d,%d]\n", seed.queryOffset, (queryPosition -
  // PSSMatrix.matrix)); fflush(stdout);
  ...
  *tracebackColumn = 0;
  ...
  //	print(fasterGappedExtension_matchRow, subject, rowDropoff,
  // columnDropoff);
  ...
  dpResults.traceback = fasterGappedExtension_traceback_multi[queryNum];
...
struct dpResults fasterGappedExtension_dpAfterSeed_multi(
  ...
  // values
  // If current malloced rows aren't big enough
  if (subjectLength >= fasterGappedExtension_rowSizes_multi[queryNum]) {
    // Free existing rows
    free(fasterGappedExtension_matchRow_multi[queryNum]);
    free(fasterGappedExtension_insertQrow_multi[queryNum]);
    free(fasterGappedExtension_insertSrow_multi[queryNum]);
    // Set size to double current needed length
    fasterGappedExtension_rowSizes_multi[queryNum] = subjectLength * 2;
    // Malloc new rows
    fasterGappedExtension_matchRow_multi[queryNum] = (int4 *)global_malloc(
        sizeof(int4) * fasterGappedExtension_rowSizes_multi[queryNum]);
    fasterGappedExtension_insertQrow_multi[queryNum] = (int4 *)global_malloc(
  ...
  // Increase number of columns in traceback array if neccessary
  if (subjectLength > fasterGappedExtension_numColumns_multi[queryNum]) {
    // For each existing row
    queryDistance = 0;
    while (queryDistance < fasterGappedExtension_numRows_multi[queryNum]) {
      // Increase number of columns
    ...
    // Update number of columns
    fasterGappedExtension_numColumns_multi[queryNum] = subjectLength;
  }

  // If more rows are required
  if (queryLength > fasterGappedExtension_numRows_multi[queryNum]) {
    // Increase number of row pointers
    fasterGappedExtension_traceback_multi[queryNum] =
        (unsigned char **)global_realloc(
            fasterGappedExtension_traceback_multi[queryNum],
    ...
    // Declare new rows
    while (fasterGappedExtension_numRows_multi[queryNum] < queryLength) {
      fasterGappedExtension_traceback_multi
          [queryNum][fasterGappedExtension_numRows_multi[queryNum]] =
              (unsigned char *)global_malloc(
                  sizeof(unsigned char) *
  ...
  // Initialize rows
  matchRow = fasterGappedExtension_matchRow_multi[queryNum] + 1;
  insertQrow = fasterGappedExtension_insertQrow_multi[queryNum] + 1;
  insertSrow = fasterGappedExtension_insertSrow_multi[queryNum] + 1;
  ...
  *tracebackColumn = 0;
  ...
  //    if (dloc==88197331)
  //    print2(fasterGappedExtension_matchRow, subject, rowDropoff,
  // columnDropoff);
...
void fasterGappedExtension_free_multi(int queryNum) {
  ...
  while (fasterGappedExtension_numRows_multi[queryNum] > 0) {
    fasterGappedExtension_numRows_multi[queryNum]--;
    free(fasterGappedExtension_traceback_multi
             [queryNum][fasterGappedExtension_numRows_multi[queryNum]]);
  }

  free(fasterGappedExtension_traceback_multi[queryNum]);
  free(fasterGappedExtension_matchRow_multi[queryNum]);
  free(fasterGappedExtension_insertQrow_multi[queryNum]);
  free(fasterGappedExtension_insertSrow_multi[queryNum]);
  fasterGappedExtension_matchRow_multi[queryNum] = NULL;
  fasterGappedExtension_insertQrow_multi[queryNum] = NULL;
  fasterGappedExtension_insertSrow_multi[queryNum] = NULL;
  fasterGappedExtension_rowSizes_multi[queryNum] = 0;
  fasterGappedExtension_traceback_multi[queryNum] = NULL;
  fasterGappedExtension_numRows_multi[queryNum] = 0;
  fasterGappedExtension_numColumns_multi[queryNum] = 0;
}
