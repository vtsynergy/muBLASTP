...
// On file for reading AND writing and map to memory, then return mapped memory
// address
void *writeFile_open(char *filename) {
  ...
  writeFile_address = mmap(0, writeFile_fileSize, PROT_WRITE, MAP_SHARED,
                           writeFile_fileDescriptor, 0);
...
size_t writeFile_getSize() { return writeFile_fileSize; }
