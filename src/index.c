...
int4 alignments_compareQueryPosition(const void *queryWord1,
                                     const void *queryWord2);
...
void index_initializeBuild(uint4 fromCodeword, uint4 toCodeword) {
  ...
  index_words = (struct wordList *)global_malloc(sizeof(struct wordList) *
                                                 (toCodeword - fromCodeword));
...
void index_addSubject(unsigned char *subject, uint4 subjectLength,
  ...
  while (wordOffset + index_wordSize - 1 < subjectLength) {
    ...
    if (codeword >= fromCodeword && codeword < toCodeword) {
      ...
      index_addWord(codeword, index_subjectNumber,
                    wordOffset /
                        index_intervalSize); // + (index_wordSize - 4));
    }
...
void index_addWord(uint4 codeword, uint4 subjectNumber, uint4 offset) {
  ...
  if (wordList->length + vbyteSize >= wordList->allocated) {
    ...
    wordList->offsets = (unsigned char *)global_realloc(
        wordList->offsets, sizeof(char) * wordList->allocated);
  }
...
// Given a query sequence uses an inverted index of the collection to identify
// the
...
void index_processQuery(unsigned char *startIndex, struct PSSMatrix PSSMatrix,
  ...
  time = clock();
  unsortedCoordinates =
      memBlocks_initialize(sizeof(struct indexCoordinate), numSequences);
  ...
  queryWords = (struct queryWord *)global_malloc(sizeof(struct queryWord) *
                                                 numQueryPositions);
  ...
  while (queryPosition < numQueryPositions) {
    ...
    while (wordPosition < index_wordSize) {
      if (PSSMatrix.queryCodes[queryPosition + wordPosition] >=
    ...
    if (!containsWildcard &&
      ...
      // index_loadedWords[codeword],
      //                   index_loadedWords[codeword + 1], endOffsets -
      // offsets);
    } else {
  ...
  qsort(queryWords, numQueryPositions, sizeof(struct queryWord),
        alignments_compareCodeword);
  ...
  while (queryWordCount < numQueryPositions) {
    ...
    if (queryWords[queryWordCount].offsets != NULL) {
      ...
      numOffsets = queryWords[queryWordCount].endOffsets -
                   queryWords[queryWordCount].offsets;
  ...
  qsort(queryWords, numQueryPositions, sizeof(struct queryWord),
        alignments_compareQueryPosition);
  ...
  while (queryPosition < numQueryPositions) {
    ...
    if (queryWords[queryPosition].offsets != NULL) {
      ...
      while (offsets < endOffsets) {
        ...
        coordinate =
            (struct indexCoordinate *)memBlocks_newEntry(unsortedCoordinates);
        coordinate->queryOffset = queryPosition4;
        coordinate->subjectOffset =
            offset * index_intervalSize + (index_wordSize - 4);
  ...
  while ((coordinate = memBlocks_getCurrent(unsortedCoordinates)) != NULL) {
    ...
    //    	printf("%d,%d=[%d]\n",
    // index_sequenceCoordinates[sequenceNumber],
    // numSubjectHits[sequenceNumber], sequenceNumber);
    // Place into sorted list
    index_sequenceCoordinates[sequenceNumber][numSubjectHits[sequenceNumber]] =
        *coordinate;
    numSubjectHits[sequenceNumber]++;
  }
...
// Compare the two queryWords' codewords
int4 alignments_compareCodeword(const void *queryWord1,
                                const void *queryWord2) {
  const struct queryWord *q1, *q2;
...
void index_print() {
  ...
  while (codeword < index_numWords) {
    ...
    //    	printf("[%d/%d = %f]\n", wordList->length, numOffsets,
    // (float)(wordList->length) / (float)numOffsets);
