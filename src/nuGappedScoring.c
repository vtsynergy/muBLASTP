...
// Prototypes
void nuGappedScoring_printBeforeRow(int4 *row, int2 **query, int2 **rowDropoff,
                                    int2 **columnDropoff);
void nuGappedScoring_printAfterRow(int4 *row, int2 **query, int2 **rowDropoff,
                                   int2 **columnDropoff);
...
int4 nuGappedScoring_score(struct ungappedExtension *ungappedExtension,
  ...
  beforeDpResults =
      nuGappedScoring_dpBeforeSeed(subject, PSSMatrix, seed, dropoff);
  ...
  if (parameters_verboseDloc == blast_dloc)
    printf("norm[%d,%d,%d](seed=%d,%d)\n", beforeDpResults.bestScore,
           ...
           choppedPSSMatrix.matrix[0][encoding_extractBase(
               choppedSubject[0], seed.subjectOffset % 4)],
           seed.queryOffset, seed.subjectOffset);
#endif

  //	printf("Start=%d,%d End=%d,%d\n", beforeDpResults.best.queryOffset,
  // beforeDpResults.best.subjectOffset,
  //                                      afterDpResults.best.queryOffset,
