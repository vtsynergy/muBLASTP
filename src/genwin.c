...
char *blastdbs[] = { "bba",       "bbn",       "embl",     "gbupdate",
                     "genbank",   "genpept",   "gpupdate", "nr",
                     "nrdb",      "nrdb.shuf", "pir",      "pseq",
                     "swissprot", "tfdaa" };
...
extern struct Database *opendbase(name) char *name;
...
extern closedbase(dbase) struct Database *dbase;
...
extern struct Sequence *firstseq(dbase) struct Database *dbase;
...
extern struct Sequence *nextseq(dbase) struct Database *dbase;

{ return (readentry(dbase)); }

/*--------------------------------------------------------------(closeseq)---*/

extern closeseq(seq) struct Sequence *seq;
...
extern struct Sequence *openwin(parent, start, length) struct Sequence *parent;
int start, length;
...
extern struct Sequence *nextwin(win, shift) struct Sequence *win;
int shift;
...
extern int shiftwin1(win) struct Sequence *win;
{
...
extern closewin(win) struct Sequence *win;
...
extern compon(win) struct Sequence *win;
{
  register int *comp;
  register int aa;
...
static int state_cmp(s1, s2) int *s1, *s2;
{ return *s2 - *s1; }

extern stateon(win) struct Sequence *win;
{
...
extern enton(win) struct Sequence *win;
...
void entropy_init(window) int window;
{
  int i;
  double x, xw;
...
extern double entropy(sv) register int *sv;
{
  int *sv0 = sv;
  register double ent;
...
static void decrementsv(sv, class) register int *sv;
register int class;
{
  register int svi;
...
static void incrementsv(sv, class) register int *sv;
int class;
{
  for (;;) {
    if (*sv++ == class) {
...
struct Sequence *readentry(dbase) struct Database *dbase;
...
extern int findchar(str, chr) char *str;
char chr;
...
extern upper(string, len) register char *string;
size_t len;
{
...
extern lower(string, len) char *string;
size_t len;
{
  register char *stringmax, c;
