...
// Prototypes
struct dpResults
fasterBytepackGappedScoring_dpBeforeSeed(unsigned char *subject,
                                         struct PSSMatrix PSSMatrix,
                                         struct coordinate seed, int4 dropoff);
struct dpResults
fasterBytepackGappedScoring_dpAfterSeed(unsigned char *subject,
                                        struct PSSMatrix PSSMatrix,
...
fasterBytepackGappedScoring_dpBeforeSeed(unsigned char *subject,
  ...
  if (seed.queryOffset >= fasterBytepackGappedScoring_rowSizes) {
    ...
    // Malloc new rows
    fasterBytepackGappedScoring_bestRows[0] = (int4 *)global_malloc(
        sizeof(int4) * fasterBytepackGappedScoring_rowSizes);
    fasterBytepackGappedScoring_bestRows[1] = (int4 *)global_malloc(
        sizeof(int4) * fasterBytepackGappedScoring_rowSizes);
  }
  ...
  if (parameters_verboseDloc == blast_dloc)
    gappedExtension_printBeforeRow(
        fasterBytepackGappedScoring_bestRows[bestRowToggle],
  ...
  while (subjectPosition > subject && rowDropoff < columnDropoff) {
    ...
    previousBestRow =
        fasterBytepackGappedScoring_bestRows[bestRowToggle] + rowOffset;
