...
void cluster_printSequence(uint4 whitespace, unsigned char *sequence,
                           uint4 length);
...
uint4 cluster_overlap(struct sequence *sequence1, struct sequence *sequence2,
                      int4 relativeOffset);
...
void cluster_addChild(struct parent *parent, struct sequence *child,
                      int4 relativeOffset);
// Merge two clusters together into one with a common parent
void cluster_mergeParents(struct parent *parent1, struct parent *parent2,
                          int4 relativeOffset);
...
uint4 cluster_checkMerge(struct wordLocation *wordLocations1,
                         ...
                         uint4 numWordLocations2, int4 *bestDiagonal);
...
// Given a list of sequences containing a word, perform a multiple alignment and
// find
// the optimal cluster
int cluster_buildCluster(struct wordLocation *wordLocations,
                         uint4 numWordLocations, struct sequence *sequences);
...
struct edit *cluster_getEdits(struct parent *parent, struct sequence *child,
                              uint4 *numEdits);
// Build a PSSM from a list of sequences containing a word
int cluster_buildPSSM(struct wordLocation *wordLocations,
                      uint4 numWordLocations, struct sequence *sequences);
// Calculate the average best score for aligning to a set of residues defined by
// given wildcode
...
int4 main(int4 argc, char *argv[]) {
  ...
  if (readdb_dbAlphabetType != encoding_protein) {
    fprintf(stderr,
            "Error: Only protein collection clustering currently supported\n");
    exit(-1);
  }

  if (readdb_numberOfSequences != readdb_numberOfClusters) {
    fprintf(stderr,
            "Error clustering %s: collection has already been clustered\n",
            filename);
    exit(-1);
  }

  sequences = (struct sequence *)global_malloc(sizeof(struct sequence) *
                                               readdb_numberOfSequences);
  ...
  // Move through index file reading sequence codes and converting back to ASCII
  // sequences
  sequenceNumber = 0;
  ...
  // Simple approach to clustering sequences
  //    cluster_simpleClusterSequences(sequences, sequenceNumber,
  // readdb_numberOfLetters);

  // Perform SPEX algorithm and cluster sequences
  cluster_spexClusterSequences(sequences, sequenceNumber,
                               readdb_numberOfLetters);

  // Cluster sequences in the collection
  //    cluster_clusterSequences(sequences, sequenceNumber,
  // readdb_numberOfLetters, filename);
...
void cluster_writeClusters(char *filename, struct sequence *sequences) {
  ...
  uint4 count, sequenceNumber, sequenceLength, descriptionLength,
      descriptionLocation;
  ...
  while ((parent = memBlocks_getCurrent(cluster_parents)) != NULL) {
    if (parent->children != NULL && parent->numChildren > 1) {
      ...
      children =
          (struct child *)global_malloc(sizeof(struct child) * numChildren);
      ...
      while (childNum < numChildren) {
        ...
        description =
            descriptions_getDescription(descriptionLocation, descriptionLength);
        ...
        children[childNum].regionStart =
            parent->children[childNum]->regionStart;
        children[childNum].length = parent->children[childNum]->length;
        children[childNum].edits = cluster_getEdits(
            parent, parent->children[childNum], &(children[childNum].numEdits));
        childNum++;
      }

      // Add sequence to the formatted collection
      writedb_addSequence(sequence, sequenceLength, NULL, 0, NULL, 0, children,
                          numChildren);
  ...
  wildcards = (struct wild *)global_malloc(sizeof(struct wild) *
  ...
  wildcards_scoreCandidates(wildcards, wildcards_numClusterWildcards, wilds,
                            numWilds, 0);
  free(wilds);
...
struct edit *cluster_getEdits(struct parent *parent, struct sequence *child,
  ...
  *numEdits = 0;
  ...
  while (count < child->length) {
    if (parent->sequence[count + child->regionStart] !=
        child->sequence[count]) {
...
void cluster_spexClusterSequences(struct sequence *sequences,
    ...
    while (sequenceNumber < numberOfSequences) {
      ...
      if (sequenceSize >= wordLength) {
        ...
        while (offset < sequenceSize - wordLength) {
          ...
          hashcounter_hash(hashCounter2, word, wordLength, hashValue2,
                           position);
          ...
          if (lookup == 1) {
            ...
            if (hashCounter1 != NULL) {
              ...
              while (suboffset <= offset + decoQantum) {
                ...
                hashcounter_hash(hashCounter1, word, wordLength - decoQantum,
                                 hashValue1, position);
            ...
            if (sinceChange >= decoQantum &&
                (hashCounter1 == NULL || submatches >= 2)) {
              hashcounter_insert(hashCounter2, hashValue2);
              sinceChange = 0;
            }
          }
  ...
  wordLocations = (struct wordLocation *)global_malloc(
      sizeof(struct wordLocation) * numberOfSequences);

  // Initialize diagonals
  diagonals = (struct memSingleBlock *)global_malloc(
      sizeof(struct memSingleBlock) * numberOfSequences);
  sequenceNumber = 0;
  while (sequenceNumber < numberOfSequences) {
    memSingleBlock_initializeExisting(diagonals + sequenceNumber,
                                      sizeof(struct diagonal), 1);
    sequenceNumber++;
  }
  ...
  while (hashtablePass < numHashtablePasses) {
      ...
      if (sequenceSize >= wordLength) {
        ...
        while (offset < sequenceSize - wordLength) {
          ...
          if (passhash % numHashtablePasses == hashtablePass) {
            ...
            word = sequence + offset;
            hashcounter_hash(hashCounter1, word, wordLength, hashValue1,
                             position);
            if (hashcounter_multiple(hashCounter1, hashValue1)) {
              postings_addEntry(sequence + offset, wordLength, sequenceNumber,
                                offset);
            }
          }
    ...
    while (key >= 0) {
      ...
      if (numWordLocations > maxListLength) {
        ...
        numWordLocations =
            cluster_removeChildren(wordLocations, numWordLocations, sequences);
        ...
        if (numWordLocations >= maxListLength &&
          ...
          while (numWordLocations >= maxListLength) {
            ...
            numWordLocations = cluster_removeChildren(
                wordLocations, numWordLocations, sequences);
            //                        printf("Now %d\n", numWordLocations);
            // fflush(stdout);
          }
  ...
  sequenceMatches = memSingleBlock_initialize(sizeof(struct sequenceMatch),
                                              numberOfSequences);
  ...
  while (sequenceNumber < numberOfSequences) {
    ...
    while ((diagonalMatch = memSingleBlock_getCurrent(sequenceDiagonals)) !=
      ...
      // Calculate rough score based on number of matches, length of overlap
      // region
      score = 100 * diagonalMatch->numMatches /
                  cluster_overlap(seq1, seq2, relativeOffset) +
              decoWordLength;

      if (score > percentMatchesRequired) // && relativeOffset == 0 &&
                                          // seq1->length == seq2->length)
      {
        // Compare the two sequences in detail
        score = cluster_score(seq1, seq2, relativeOffset, wildcardsThreshold);
        if (score > 0) {
          //                    printf("Sequences %d,%d lengths %d,%d offset %d
          // matches=%d score=%d\n",
          //                           sequence1, sequence2,
          // sequences[sequence1].length, sequences[sequence2].length,
          //                           relativeOffset,
          // diagonalMatch->numMatches, score);
...
void cluster_simpleClusterSequences(struct sequence *sequences,
                                    uint4 numberOfSequences,
                                    uint4 numberOfLetters) {
  uint4 sequenceLength = 0, sequenceCount, startGroup, endGroup,
        numWordLocations;
...
void cluster_clusterSequences(struct sequence *sequences,
                              uint4 numberOfSequences, uint4 numberOfLetters,
                              char *filename) {
  struct wordLocation *wordLocations;
  uint4 numWordLocations;
...
void cluster_processSequenceMatches(struct memSingleBlock *sequenceMatches,
  ...
  qsort(sequenceMatches->block, sequenceMatches->numEntries,
        sizeof(struct sequenceMatch), cluster_compareScore);
  ...
  while ((sequenceMatch = memSingleBlock_getCurrent(sequenceMatches)) != NULL) {
    ...
    //    	printf("sequence match %d,%d\n", sequenceMatch->sequence1,
    // sequenceMatch->sequence2);
    ...
    // Case 2 - sequence1 has a parent, sequence2 does not
      ...
      //	printf("Add to %d\n", seq1->parent->numChildren);
      // fflush(stdout);

      // Check score for aligning sequence to parent
      sequenceMatch->relativeOffset += seq1->regionStart;
      score = cluster_score(seq1, seq2, sequenceMatch->relativeOffset,
                            wildcardsThreshold);
      //			printf("[%d][%d,%d,%d]\n", score,
      // seq1->parent->length, seq2->length, sequenceMatch->relativeOffset);
    ...
    // Case 3 - both sequences have parents
      ...
      // seq2->parent->numChildren); fflush(stdout);
      ...
      score = cluster_score(seq1, seq2, sequenceMatch->relativeOffset,
                            wildcardsThreshold);
      if (score > 0) {
        cluster_mergeParents(seq1->parent, seq2->parent,
                             sequenceMatch->relativeOffset);
      }
    }
  }
...
void cluster_processList(struct memSingleBlock *diagonals,
  ...
  while (count1 < numWordLocations) {
    ...
    while (count2 < numWordLocations) {
      // location1 < location2
      if (wordLocations[count1].sequenceNumber <
          wordLocations[count2].sequenceNumber) {
      ...
      if (location1.sequenceNumber != location2.sequenceNumber) {
        // Search list of matching diagonals for this sequence for one that is
        // the
        ...
        // Otherwise create a new entry, ONLY if neither sequence1 or 2 belong
        // to a cluster
        if (!matchFound) // &&
            // sequences[wordLocations[count1].sequenceNumber].parent
            // == NULL
            // && sequences[wordLocations[count2].sequenceNumber].parent ==
            // NULL)
        {
...
int cluster_buildPSSM(struct wordLocation *wordLocations,
  ...
  while (column < cluster_PSSMend) {
    ...
    cluster_PSSM[column] =
        (uint4 *)global_malloc(sizeof(uint4) * encoding_numCodes);
    code = 0;
  ...
  while (locationNum < numWordLocations) {
    if (sequences[wordLocations[locationNum].sequenceNumber].parent == NULL) {
      ...
      while (column < sequence.length) {
        //            	printf("[%d]", sequence.sequence[column]);
        // fflush(stdout);
...
int cluster_buildCluster(struct wordLocation *wordLocations,
  ...
  while (locationNum < numWordLocations) {
    if (sequences[wordLocations[locationNum].sequenceNumber].parent == NULL) {
      ...
      //            printf("[%3d] ", score);
      //            cluster_printSequence(start - cluster_PSSMstart,
  ...
  parent = (struct parent *)memBlocks_newEntry(cluster_parents);
  cluster_newParent(parent,
                    sequences + wordLocations[bestLocation].sequenceNumber);
...
// Remove any duplicate locations in the list that refer to children from the
// same parent;
...
uint4 cluster_removeChildren(struct wordLocation *wordLocations,
  ...
  wordLocations2 = (struct wordLocation *)global_malloc(
      sizeof(struct wordLocation) * numWordLocations);
  numWordLocations2 = 0;
  locationNum = 0;
  while (locationNum < numWordLocations) {
  ...
  memcpy(wordLocations, wordLocations2,
         sizeof(struct wordLocation) * numWordLocations2);
  free(wordLocations2);
...
void cluster_newParent(struct parent *parent, struct sequence *child) {
  ...
  parent->children =
      (struct sequence **)global_malloc(sizeof(struct sequence *));
...
void cluster_updateWildcode(struct parent *parent, uint4 position,
                            uint4 wildCode) {
  uint4 count;
  ...
  parent->numWildcards++;
  parent->wildCodes = (struct wildCode *)global_realloc(
      parent->wildCodes, parent->numWildcards * sizeof(struct wildCode));
...
void cluster_mergeParents(struct parent *parent1, struct parent *parent2,
  ...
  if (parent2->length + relativeOffset > parent1->length) {
    ...
    parent1->sequence = (char *)global_realloc(
        parent1->sequence, parent2->length + relativeOffset);
    memcpy(parent1->sequence + parent1->length,
           parent2->sequence + parent1->length - relativeOffset,
...
void cluster_addChild(struct parent *parent, struct sequence *child,
  ...
  if (relativeOffset < 0) {
    ...
    memcpy(newParentSequence - relativeOffset, parent->sequence,
           parent->length);
  ...
  if (child->length + relativeOffset > parent->length) {
    ...
    parent->sequence = (char *)global_realloc(parent->sequence,
                                              child->length + relativeOffset);
    memcpy(parent->sequence + parent->length,
           child->sequence + parent->length - relativeOffset,
  ...
  count = 0;
  while (count < child->length) {
    //    	printf("[%d/%d]", count + relativeOffset, parent->length);
    // fflush(stdout);
    // If characters don't match
    if (child->sequence[count] != parent->sequence[count + relativeOffset]) {
      //        	printf("%d: %d,%d\n", count + relativeOffset,
      // child->sequence[count],
      //                                  parent->sequence[count +
      // relativeOffset]); fflush(stdout);

      if (parent->sequence[count + relativeOffset] >=
          encoding_aaStartWildcards) {
        // Get parent wildcode
      ...
      if (!getbit(wildCode, child->sequence[count])) {
        ...
        parent->sequence[count + relativeOffset] =
            encoding_aaStartWildcards + wildcardCount;
      }
  ...
  child->parent = parent;
  parent->numChildren++;
  parent->children = (struct sequence **)global_realloc(
      parent->children, sizeof(struct sequence *) * parent->numChildren);
...
uint4 cluster_score(struct sequence *seq1, struct sequence *seq2,
  ...
  while (pos1 < end1) {
    ...
    if (sequence1[pos1] != sequence2[pos2]) {
      ...
      //                   encoding_getLetter(sequence2[pos2]), parent1,
      // parent2);

      // Sequence1 char is included in parent2's wildcard
      if (parent1 == NULL && parent2 != NULL &&
          sequence2[pos2] >= encoding_aaStartWildcards &&
          getbit(wildcards_clusterWildcards
                     [sequence2[pos2] - encoding_aaStartWildcards].wildCode,
                 sequence1[pos1])) {
        wildCode = wildcards_clusterWildcards
                       [sequence2[pos2] - encoding_aaStartWildcards].wildCode;
      }
      // Sequence2 char is included in parent1's wildcard
               ...
               getbit(wildcards_clusterWildcards
                      ...
                      sequence2[pos2])) {
        wildCode = wildcards_clusterWildcards
                       [sequence1[pos1] - encoding_aaStartWildcards].wildCode;
      } else {
      ...
      sumWindowWildscores +=
          wildcards_clusterWildcards[wildcardCount].averageScore -
          cluster_averageMatchScore;

      /*            // Add wild's average score to tally
                  if (wildcards_clusterWildcards[wildcardCount].averageScore >
         0)
                  {
                      sumWildscores +=
         wildcards_clusterWildcards[wildcardCount].averageScore;
  ...
  // Return score
  //    return (end1 - start1) * (wildscoreAllowed - sumWildscores) /
  // wildscoreAllowed;
...
float cluster_averageWildcodeScore(uint4 wildCode) {
  ...
  if (cluster_averageWildcodeScores == NULL) {
    cluster_averageWildcodeScores = (float *)global_malloc(
        sizeof(float) * ceil(pow(2, encoding_numLetters)));
    tempCode = 0;
