...
unsigned char readNcbidb_codes[readNcbidb_numCodes] = {
  31, 1, 20, 18, 8, 5, 14, 2,  17, 10, 7,  0, 16,
  12, 9, 13, 11, 3, 6, 4,  19, 22, 15, 21, 23
};
...
void readNcbidb_open(char *filename) {
  ...
  if (fread(created, sizeof(char), createdLength, readNcbidb_pinFile) <
      createdLength) {
    fprintf(stderr, "Error reading from file %s\n", readNcbidb_pinFilename);
    exit(-1);
  }
  ...
  readNcbidb_numberOfSequences =
      readNcbidb_readInt(readNcbidb_pinFile, readNcbidb_pinFilename);
  readNcbidb_numberOfLetters =
      readNcbidb_read64Int(readNcbidb_pinFile, readNcbidb_pinFilename);
  readNcbidb_longestSequenceLength =
      readNcbidb_readInt(readNcbidb_pinFile, readNcbidb_pinFilename);

  //    printf("%d,%llu,%d\n", readNcbidb_numberOfSequences,
  // readNcbidb_numberOfLetters, readNcbidb_longestSequenceLength);

  // Read description offsets
  readNcbidb_descriptionOffsets =
      global_malloc(sizeof(uint4) * (readNcbidb_numberOfSequences + 1));
  sequenceNumber = 0;
...
unsigned char *readNcbidb_readASN1sequenceLength(unsigned char *data,
                                                 uint4 *length) {
  int lengthLength;
...
unsigned char *readNcbidb_getDescription(uint4 sequenceNumber, uint4 *length) {
  ...
  *length = readNcbidb_descriptionOffsets[sequenceNumber + 1] -
  ...
  descriptionStart =
      readNcbidb_readASN1sequenceLength(descriptionStart, length);
