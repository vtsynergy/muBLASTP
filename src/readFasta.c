...
void readFasta_open(char *filename) {
  ...
  readFasta_sequenceBuffer =
      (char *)global_malloc(sizeof(char) * readFasta_bufferAlloc);
  readFasta_descriptionBuffer =
      (char *)global_malloc(sizeof(char) * readFasta_bufferAlloc);
  readFasta_lineBuffer =
      (char *)global_malloc(sizeof(char) * readFasta_bufferAlloc);
  readFasta_lineBuffer[0] = '\0';
  readFasta_lineBuffer[1] = '\0';
  readFasta_lineLength = 1;
  readFasta_endReached = 0;
}

// Read a description and sequence from file, return 0 if end of file, otherwise
// 1
int4 readFasta_readSequence() {
  ...
  memcpy(readFasta_descriptionBuffer, readFasta_lineBuffer + 1,
         readFasta_lineLength);
  readFasta_descriptionLength = readFasta_lineLength - 1;

  // Read each line from FASTA file
  while (!readFasta_endReached &&
         fgets(readFasta_lineBuffer, readFasta_bufferAlloc, readFasta_file)) {
    ...
    while (readFasta_lineBuffer[readFasta_lineLength - 1] != '\n') {
      ...
      readFasta_lineBuffer = (char *)global_realloc(
          readFasta_lineBuffer, sizeof(char) * readFasta_bufferAlloc * 2);
      readFasta_descriptionBuffer =
          (char *)global_realloc(readFasta_descriptionBuffer,
                                 sizeof(char) * readFasta_bufferAlloc * 2);

      // Read next line
      if (!fgets(readFasta_lineBuffer + readFasta_bufferAlloc - 1,
                 readFasta_bufferAlloc, readFasta_file)) {
    ...
    // Otherwise it is part of a sequence
      ...
      if (readFasta_lineLength + readFasta_sequenceLength + 1 >
          readFasta_sequenceBufferAlloc) {
        // Increase size of the buffer
        readFasta_sequenceBufferAlloc =
            (readFasta_lineLength + readFasta_sequenceLength + 1) * 2;
        readFasta_sequenceBuffer = (char *)global_realloc(
            readFasta_sequenceBuffer,
      ...
      while (pos < readFasta_lineLength) {
        if ((readFasta_lineBuffer[pos] >= 'a' &&
             readFasta_lineBuffer[pos] <= 'z') ||
            (readFasta_lineBuffer[pos] >= 'A' &&
             readFasta_lineBuffer[pos] <= 'Z')) {
      ...
      memcpy(readFasta_sequenceBuffer + readFasta_sequenceLength,
             readFasta_lineBuffer, readFasta_lineLength + 1);
