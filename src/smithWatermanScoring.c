...
// Prototypes
struct dpResults smithWatermanScoring_dynamicProgramming(
    unsigned char *subject, struct PSSMatrix PSSMatrix, int4 subjectLength);
#define maximum(a, b) ((a > b) ? a : b)

// Perform smith-waterman dynamic programming on the given query and subject
// sequences
struct dpResults smithWatermanScoring_score(struct PSSMatrix PSSMatrix,
  ...
  if (PSSMatrix.strandLength != PSSMatrix.length) {
    ...
    dpResults = smithWatermanScoring_dynamicProgramming(
        subject, choppedPSSMatrix, subjectSize);

    // Perform dynamic programming on second strand
    choppedPSSMatrix = PSSMatrix_chop(PSSMatrix, PSSMatrix.strandLength);
    dpResults2 = smithWatermanScoring_dynamicProgramming(
        subject, choppedPSSMatrix, subjectSize);
    ...
    if (dpResults.bestScore > dpResults2.bestScore)
      ...
      //        	printf("[%d:%d,%d]", dpResults2.bestScore,
      // dpResults2.best.queryOffset,
      //                                 dpResults2.best.subjectOffset);
      return dpResults2;
    }
  } else {
    // Perform dynamic programming
    dpResults = smithWatermanScoring_dynamicProgramming(subject, PSSMatrix,
                                                        subjectSize);

    //    	printf("Best=%d,%d Score=%d dloc=%d\n",
    // dpResults.best.queryOffset,
    // dpResults.best.subjectOffset,
    //    	                                        dpResults.bestScore,
    // blast_dloc); fflush(stdout);

    return dpResults;
  }
}

// Reverse the query and subject sequences, and perform dynamic programming to
// find the
...
struct dpResults smithWatermanScoring_scoreReverse(struct PSSMatrix PSSMatrix,
  ...
  while (subjectPosition < subjectSize) {
    reversedSubject[subjectPosition] =
        subject[subjectSize - subjectPosition - 1];
    subjectPosition++;
  }
  ...
  dpResults = smithWatermanScoring_score(reversedPSSMatrix, subjectSize,
                                         reversedSubject);

  dpResults.best.queryOffset =
      PSSMatrix.length - dpResults.best.queryOffset - 1;
...
struct dpResults smithWatermanScoring_dynamicProgramming(
  ...
  if (subjectLength >= smithWatermanScoring_rowSizes) {
    ...
    // Malloc new rows
    smithWatermanScoring_bestRow =
        (int4 *)global_malloc(sizeof(int4) * smithWatermanScoring_rowSizes);
    smithWatermanScoring_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * smithWatermanScoring_rowSizes);
  }
  ...
  start:
    ...
    //		if (dloc==72249)
    //			smithWatermanTraceback_print(smithWatermanScoring_bestRow,
    // subjectLength);
  }
