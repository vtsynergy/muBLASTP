...
struct PSSMatrix PSSMatrix_create(struct scoreMatrix scoreMatrix, char *query) {
  ...
  } else if (encoding_alphabetType == encoding_nucleotide &&
             parameters_strands == 2) {
  ...
  PSSMatrix.matrix =
      (int2 **)global_malloc(sizeof(int2 *) * (PSSMatrix.length + 2));

  // Declare memory to store encoded query
  PSSMatrix.queryCodes = (unsigned char *)global_malloc(sizeof(unsigned char) *
                                                        (PSSMatrix.length));

  // Declare memory to store the highest scoring regular letter for each
  // position
  PSSMatrix.bestMatchCodes = (unsigned char *)global_malloc(
      sizeof(unsigned char) * (PSSMatrix.length));
  ...
  while (columnCount < PSSMatrix.length) {
    ...
    if (columnCount < PSSMatrix.strandLength) {
      ...
      // Negative strand
      code = encoding_getComplement(
          encoding_getCode(query[PSSMatrix.length - columnCount - 1]));
    }
    ...
    while (code2 < encoding_numRegularLetters) {
      ...
      if (PSSMatrix.matrix[columnCount][code2] >
          PSSMatrix.matrix[columnCount][bestCode]) {
        bestCode = code2;
      }
      code2++;
    }
    ...
    if (encoding_alphabetType == encoding_nucleotide &&
        code >= encoding_numRegularLetters) {
      PSSMatrix.bestMatchCodes[columnCount] =
          encoding_randomEncodedLetter(code);
    } else {
  ...
  if (encoding_alphabetType == encoding_nucleotide) {
    // Declare memory to store xor codes; each 2-bit code copied 4 times in the
    // byte
    PSSMatrix.xorCodes = (unsigned char *)global_malloc(sizeof(unsigned char) *
                                                        (PSSMatrix.length));
    ...
    while (columnCount < PSSMatrix.length) {
      ...
      PSSMatrix.xorCodes[columnCount] =
          (code << 6) | code << 4 | code << 2 | code;
    ...
    while (columnCount < PSSMatrix.length) {
      ...
      // Of the remaining < 4 characters
        ...
        PSSMatrix.bytePackedCodes[columnCount] =
            encoding_bytePackRemaining(PSSMatrix.bestMatchCodes + columnCount,
                                       PSSMatrix.length - columnCount);
    ...
    PSSMatrix.bytePackedCodes[-3] =
        encoding_bytePackBeginning(PSSMatrix.bestMatchCodes, 1);
    PSSMatrix.bytePackedCodes[-2] =
        encoding_bytePackBeginning(PSSMatrix.bestMatchCodes, 2);
    PSSMatrix.bytePackedCodes[-1] =
        encoding_bytePackBeginning(PSSMatrix.bestMatchCodes, 3);

    PSSMatrix_packedRightMatches =
        (unsigned char *)global_malloc(sizeof(unsigned char) * 256);
    PSSMatrix_packedLeftMatches =
        (unsigned char *)global_malloc(sizeof(unsigned char) * 256);
    PSSMatrix_packedRightMatchScores =
        (int2 *)global_malloc(sizeof(int2) * 256);
    PSSMatrix_packedLeftMatchScores = (int2 *)global_malloc(sizeof(int2) * 256);
    PSSMatrix_packedScore =
        (unsigned char *)global_malloc(sizeof(unsigned char) * 256);
    ...
    while (packedByte < 256) {
      ...
      PSSMatrix_packedRightMatchScores[packedByte] =
      ...
      PSSMatrix_packedLeftMatchScores[packedByte] =
          matches * parameters_matchScore;
  ...
  PSSMatrix.matrix[PSSMatrix.length] =
...
// Returns a PSSMatrix with the first "amount" entries removed and the length
// shortened
// accordingly
...
struct PSSMatrix PSSMatrix_reverse(struct PSSMatrix PSSMatrix) {
  ...
  reversed.matrix =
      (int2 **)global_malloc(sizeof(int2 *) * (PSSMatrix.length + 2));
  reversed.queryCodes = (unsigned char *)global_malloc(sizeof(unsigned char) *
                                                       (PSSMatrix.length));
...
void PSSMatrix_free(struct PSSMatrix PSSMatrix) {
  // Do not iterate through each column and free it, since PSSMatrix shares
  // memory
