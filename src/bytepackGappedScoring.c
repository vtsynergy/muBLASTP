...
// Prototypes
struct dpResults bytepackGappedScoring_dpBeforeSeed(unsigned char *subject,
                                                    struct PSSMatrix PSSMatrix,
                                                    struct coordinate seed,
                                                    int4 dropoff);
struct dpResults bytepackGappedScoring_dpAfterSeed(unsigned char *subject,
                                                   struct PSSMatrix PSSMatrix,
                                                   int4 dropoff,
                                                   int4 subjectLength);
...
int4 bytepackGappedScoring_score(struct ungappedExtension *ungappedExtension,
  ...
  if (parameters_bytepackStartGap == 0) {
    beforeDpResults = fasterBytepackGappedScoring_dpBeforeSeed(
        packedSubject, PSSMatrix, seed, dropoff);
  } else {
    beforeDpResults = bytepackGappedScoring_dpBeforeSeed(
        packedSubject, PSSMatrix, seed, dropoff);
  }
  ...
  if (parameters_bytepackStartGap == 0) {
    afterDpResults = fasterBytepackGappedScoring_dpAfterSeed(
        choppedSubject, choppedPSSMatrix, dropoff, choppedSubjectSize);
  } else {
    afterDpResults = bytepackGappedScoring_dpAfterSeed(
        choppedSubject, choppedPSSMatrix, dropoff, choppedSubjectSize);
  }
  ...
  if (parameters_verboseDloc == blast_dloc)
    printf("bytepacked[%d,%d,%d] (seed=%d,%d)\n", beforeDpResults.bestScore,
           ...
           PSSMatrix_packedScore
               [choppedPSSMatrix.bytePackedCodes[0] ^ choppedSubject[0]],
...
struct dpResults bytepackGappedScoring_dpBeforeSeed(unsigned char *subject,
  ...
  int4 bestScore = 0;
  int4 *bestRow, *previousBestRow, *insertQrow, insertS, rowOffset,
      bestRowToggle = 0;
  ...
  if (seed.queryOffset >= bytepackGappedScoring_rowSizes) {
    ...
    // Malloc new rows
    bytepackGappedScoring_bestRows[0] =
        (int4 *)global_malloc(sizeof(int4) * bytepackGappedScoring_rowSizes);
    bytepackGappedScoring_bestRows[1] =
        (int4 *)global_malloc(sizeof(int4) * bytepackGappedScoring_rowSizes);
    bytepackGappedScoring_insertQrow =
        (int4 *)global_malloc(sizeof(int4) * bytepackGappedScoring_rowSizes);
  }

  bestSubjectPosition = subjectPosition = subject + seed.subjectOffset;
  bestQueryPosition = queryPosition =
      PSSMatrix.bytePackedCodes + seed.queryOffset;
  ...
  *insertQrow = -parameters_bytepackOpen4Gap;
...
#ifdef VERBOSE
  if (parameters_verboseDloc == blast_dloc)
    gappedExtension_printBeforeRow(
        bytepackGappedScoring_bestRows[bestRowToggle],
        PSSMatrix.bytePackedCodes, rowDropoff, columnDropoff);
#endif
  ...
  while (subjectPosition > subject && rowDropoff < columnDropoff) {
    ...
    while (queryPosition >= rowDropoff) {
      ...
      match = PSSMatrix_packedScore[*queryPosition ^ *subjectPosition] +
              previousBestRow[4];
...
#ifdef VERBOSE
  if (parameters_verboseDloc == blast_dloc)
    gappedExtension_printAfterRow(bytepackGappedScoring_bestRows[bestRowToggle],
                                  PSSMatrix.bytePackedCodes, rowDropoff,
                                  columnDropoff);
#endif
