...
int4 *semiGappedScoring_bestRow_multi[BATCH_SIZE] = { NULL };
int4 *semiGappedScoring_insertQrow_multi[BATCH_SIZE] = { NULL };
int4 semiGappedScoring_rowSizes_multi[BATCH_SIZE] = { 0 };

#define maximum(a, b) ((a > b) ? a : b)

// Prototypes
struct dpResults semiGappedScoring_dpBeforeSeed_multi(
    unsigned char *subject, struct PSSMatrix PSSMatrix, struct coordinate seed,
    int4 dropoff, int queryNum);
struct dpResults semiGappedScoring_dpAfterSeed_multi(unsigned char *subject,
...
int4 semiGappedScoring_score_multi(struct ungappedExtension *ungappedExtension,
  ...
  blast_numSemiGapped_multi[queryNum]++;

  // Use old code if restricted insertion has been disabled
  if (!parameters_restrictedInsertionScoring) {
    return oldSemiGappedScoring_score(ungappedExtension, PSSMatrix, subjectSize,
                                      subject, dropoff);
  }
  ...
  beforeDpResults = semiGappedScoring_dpBeforeSeed_multi(
      subject, PSSMatrix, seed, dropoff + parameters_semiGappedDropoffIncrease,
      queryNum);
  ...
  afterDpResults = semiGappedScoring_dpAfterSeed_multi(
      choppedSubject, choppedPSSMatrix,
...
struct dpResults semiGappedScoring_dpBeforeSeed_multi(
  ...
  if (seed.subjectOffset >= semiGappedScoring_rowSizes_multi[queryNum]) {
#ifdef PREALLOC
      fprintf(stderr, "Error: subjectOffset:%d larger than SEMIGAPPED_ROWSIZE: %d\n", seed.subjectOffset, SEMIGAPPED_ROWSIZE);
      exit(0);
#endif
    // Free existing rows
    free(semiGappedScoring_bestRow_multi[queryNum]);
    free(semiGappedScoring_insertQrow_multi[queryNum]);
    // Set size to double current needed length
    semiGappedScoring_rowSizes_multi[queryNum] = (seed.subjectOffset) * 2;
    // Malloc new rows
    semiGappedScoring_bestRow_multi[queryNum] = (int4 *)global_malloc(
        sizeof(int4) * semiGappedScoring_rowSizes_multi[queryNum]);
    semiGappedScoring_insertQrow_multi[queryNum] = (int4 *)global_malloc(
        sizeof(int4) * semiGappedScoring_rowSizes_multi[queryNum]);
  }
  ...
  bestRow = semiGappedScoring_bestRow_multi[queryNum] + rowOffset;
  insertQrow = semiGappedScoring_insertQrow_multi[queryNum] + rowOffset;
...
struct dpResults semiGappedScoring_dpAfterSeed_multi(unsigned char *subject,
  ...
  // values
  // If current malloced rows aren't big enough
  if (subjectLength >= semiGappedScoring_rowSizes_multi[queryNum]) {
#ifdef PREALLOC
      fprintf(stderr, "Error: subjectOffset:%d larger than SEMIGAPPED_ROWSIZE: %d\n", subjectLength, SEMIGAPPED_ROWSIZE);
      exit(0);
#endif
    // Free existing rows
    free(semiGappedScoring_bestRow_multi[queryNum]);
    free(semiGappedScoring_insertQrow_multi[queryNum]);
    // Set size to double current needed length
    semiGappedScoring_rowSizes_multi[queryNum] = subjectLength * 2;
    // Malloc new rows
    semiGappedScoring_bestRow_multi[queryNum] = (int4 *)global_malloc(
        sizeof(int4) * semiGappedScoring_rowSizes_multi[queryNum]);
    semiGappedScoring_insertQrow_multi[queryNum] = (int4 *)global_malloc(
  ...
  // Initialize rows
  bestRow = semiGappedScoring_bestRow_multi[queryNum] + 1;
  insertQrow = semiGappedScoring_insertQrow_multi[queryNum] + 1;
  ...
  *insertQrow = insertS = match - parameters_semiGappedOpenGap;
  ...
  //    if (dloc == 88197331)
  //        print2(semiGappedScoring_bestRow + 1, subject, rowDropoff,
  // columnDropoff);
  ...
  while (queryPosition < queryEnd && rowDropoff > columnDropoff) {
    ...
    // Determine subjectCount for initial subjectPosition. Is used to only allow
    // insertQ when subjectOffset % parameters_semiGappedExtensionN == 0
    subjectCount = ((int4)(subjectPosition - startSubjectPosition) %
                    parameters_semiGappedExtensionN);
    if (subjectCount)
      subjectCount = parameters_semiGappedExtensionN - subjectCount;
...
void semiGappedScoring_free_multi(int queryNum) {
  free(semiGappedScoring_bestRow_multi[queryNum]);
  free(semiGappedScoring_insertQrow_multi[queryNum]);
  semiGappedScoring_bestRow_multi[queryNum] = NULL;
  semiGappedScoring_insertQrow_multi[queryNum] = NULL;
  semiGappedScoring_rowSizes_multi[queryNum] = 0;
}
