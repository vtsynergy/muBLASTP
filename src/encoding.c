...
//
// Functions for converting ASCII amino acid and nucleotide characters and
// sequences
...
unsigned char encoding_numLetters, encoding_numRegularLetters,
    encoding_numCodes;
...
#ifdef NCBI_BLAST
#define encoding_aaNumRegularLetters 24
#else
...
void encoding_initialize(unsigned char alphabetType) {
  ...
  if (encoding_alphabetType == encoding_protein) {
    ...
    encoding_packedByteLookup =
        (struct packedByte *)global_malloc(sizeof(struct packedByte) * 256);
  ...
  // Create array for translating amino acid characters into the related byte
  // code
  ...
  while (code < encoding_numLetters) {
    ...
    encoding_wildcards[code].replacementCodes =
        (char *)global_malloc(sizeof(char) * encoding_wildcards[code].numCodes);
  ...
  if (encoding_alphabetType == encoding_nucleotide) {
    encoding_complements = (unsigned char *)global_malloc(
        sizeof(unsigned char) * encoding_numLetters);

    // For each code
    code = 0;
    while (code < encoding_numLetters) {
      // Store the code of the complement
      encoding_complements[code] =
          encoding_getCode(encoding_nuComplementsArray[code]);
...
// Given a code, returns the corresponding amino acid or nucleotide character,
// or '_' if not valid
...
unsigned char encoding_determineAlphabetType(char *sequence,
  ...
  // If not all nucleotide chars or less than 90% are G,A,T,C or N this is a
  // protein query
...
unsigned char encoding_randomEncodedLetter(unsigned char code) {
  ...
  if (encoding_alphabetType == encoding_protein) {
    ...
    if (encoding_wildcards[code].replacementCounter >=
        encoding_wildcards[code].numCodes)
      encoding_wildcards[code].replacementCounter = 0;

    return encoding_wildcards[code]
        .replacementCodes[encoding_wildcards[code].replacementCounter];
  } else {
    encoding_wildcards[code].replacementCounter++;
    if (encoding_wildcards[code].replacementCounter >=
        encoding_wildcards[code].numCodes)
...
unsigned char *encoding_byteUnpackRegion(unsigned char *subject,
  ...
  while (packedCount < numPackedBytes) {
    ...
    memcpy(subjectPosition,
           encoding_packedByteLookup[bytePackedSequence[packedCount]].codes,
           sizeof(char) * 4);
...
int4 encoding_replaceWildcards(struct memSingleBlock *wildcardEdits,
                               unsigned char *sequence, int4 sequenceSize) {
...
int4 encoding_bytePackSequence(unsigned char *sequence, int4 sequenceSize) {
  ...
  if (sequenceSize % 4) {
    sequence[count] =
        encoding_bytePackRemaining(sequence + count * 4, sequenceSize % 4);
    count++;
  }
...
void encoding_encodeSequence(char *sequence, int4 sequenceSize,
                             uint4 alphabetType) {
  int4 count;
  unsigned char code;
